...
fn main() {
    ...
    let app_name = "security_closing_stock_mabs";
    ...
    info!(
        log,
        "Total Duration of Security Closing Stock Cashflow derivator: {:?}", total_duration
    );
    println!(
        "Total Duration of Security Closing Stock Cashflow derivator: {:?}",
        total_duration
    );
}

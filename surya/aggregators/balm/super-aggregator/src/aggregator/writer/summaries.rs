    ...
    for aggregates in aggregate_record.iter() {
        match aggregates {
            Some(a) => {
                ...
                int_weighted_rate_builder.build_with(a.int.rate, a.int.amount);
                slr_weighted_rate_builder.build_with(a.slr.rate, a.slr.amount);
                irs_weighted_rate_builder.build_with(a.irs.rate, a.irs.amount);
            }
            None => {}
        }
    }

...
pub fn generate(config_params: &ConfigurationParameters, log: &Logger, diag_log: &Logger) {
    ...
    let mut tot_prin_in_in: f64 = DEFAULT_FLOAT;
    let mut tot_prin_in_op: f64 = DEFAULT_FLOAT;
    let mut tot_int_in_op: f64 = DEFAULT_FLOAT;
    ...
    loop {
        ...
        if let Some(amt) = input_account.os_cost_val {
            tot_prin_in_in += amt;
        }
        ...
        tot_prin_in_op += account_with_cashflows.total_principal_amount;
        tot_int_in_op += account_with_cashflows.total_interest_amount;
        log_measurements!(
            diag_log,
            [format!(

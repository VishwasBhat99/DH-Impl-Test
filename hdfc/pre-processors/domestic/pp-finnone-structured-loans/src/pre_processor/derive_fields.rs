...
pub fn append_alm_line(
    ...
    if alm_line == "NONE" {
        get_concat_line(acc_num, gl, key)
    } else {
        String::new()
    }
}
...
fn get_concat_line(deal_no: &str, gl: &str, alm_concat: &str) -> String {
    ...
    op_line.push_str("FinStructLoans|");
...
pub fn append_next_rep_dt(
    ...
    if rep_freq == "" {
        ...
        if rate_flag.to_uppercase() == "FLOATING" {
            ...
            if rate_code.is_none() {
                ...
                if days_added_to_bus_dt != 0 {
                    ...
                    if rep_freq == "" {
                        ...
                        let mut reset_month: u32 = get_month_value(
                            &rate_code.unwrap().reset_month[..].to_uppercase().as_str(),
                        );

...
fn main() {
    ...
    let app_name = "cf_inr_irs_reg_nd";
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for INR-IRS Register.");
    info!(
        log,
        "Total Duration for INR-IRS Register: {:?}", total_duration
    );
    println!(
        "Total Duration for INR-IRS Register cashflow file generator: {:2?}",
        total_duration
    );
}

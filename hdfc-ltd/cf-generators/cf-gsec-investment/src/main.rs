...
fn main() {
    ...
    let app_name = "cf-gsec-investment-1.1.2506";
    ...
    let total_duration = end_generate_timer
        ...
        .expect("Could not calculate total duration for Gsec Investments.");
    info!(
        log,
        "Total duration for Gsec Investments cashflow generation: `{:?}`.", total_duration
    );
    println!(
        "Total Duration for Gsec Investments cashflow generation: {:2?}",
        total_duration
    );
}

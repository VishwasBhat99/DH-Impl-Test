...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for the Moneymarket Pre-Processor.");
    info!(
        log,
        "Total Moneymarket Pre-Processor Duration: {:?}", total_duration
    );
    println!(
        "Total Moneymarket Pre-Processor Duration: {:?}",
        total_duration
    );
}

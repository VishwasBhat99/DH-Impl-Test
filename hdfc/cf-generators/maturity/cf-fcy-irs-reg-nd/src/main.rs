...
fn main() {
    ...
    let app_name = "cf_fcy_irs_reg_nd";
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for FCYIRS Register.");
    info!(
        log,
        "Total Duration for FCYIRS Register: {:?}", total_duration
    );
    println!(
        "Total Duration for FCYIRS Register cashflow file generator: {:2?}",
        total_duration
    );
}

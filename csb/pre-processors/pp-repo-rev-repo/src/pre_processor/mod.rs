...
use self::structs::{alm_master::*, cust_master::*, gls::*, input_account::InputAccount};
...
pub fn process(config_param: ConfigurationParameters, log: &Logger, diag_log: &Logger) {
    ...
    let mut treas_gl_master_reader = read_file(config_param.treas_gl_master());
    let mut treas_gl_master: GLMasterMap = GLMasterMap::new();
    for (line_num, lines) in treas_gl_master_reader.deserialize().enumerate() {
        let gls: GLMaster = extract_lines(line_num, lines, config_param.cust_master(), log);
        treas_gl_master.store.insert(gls.treas_gl_cd, gls.cbs_gl_cd);
    }
    ...
    for (line_num, lines) in input_reader.deserialize().enumerate() {
        ...
        let amt = input_account.book_value.parse().unwrap_or(DEFAULT_FLOAT);
        tot_amt += amt;
        let gl_cd = get_gl_code(input_account.treas_gl_cd.to_string(), &mut treas_gl_master);
        ...
        let recon_key = ReconKey::new(input_account.ccy, String::from("RepoRevRepo"), gl_cd);
        recon
            .store
            .entry(recon_key)
    ...
    output_writer(
        &mut recon_writer,
        recon.print(*config_param.as_on_date(), "RepoRevRepo"),
    ...
    debug!(
        diag_log,
        "Writing Repo Reverse Repo, Total Duration: {:?}.", duration
    );
}

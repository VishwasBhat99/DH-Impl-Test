...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for Finware-Casa-OD-Pre-Processor.");
    info!(
        log,
        "Total Duration for Finware-Casa-OD-Pre-Processor: {:?}", total_duration
    );
    println!(
        "Total Duration for Finware-Casa-OD-Pre-Processor: {:.2?}",
        total_duration
    );
}

...
fn main() {
    ...
    // initialize loggers
    let app_name = "td-dailyseries-1.0.0";
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for TD Liquidity Daily Series.");
    info!(
        log,
        "Total Duration for TD Liquidity Daily Series: {:?}", total_duration
    );

...
fn main() {
    ...
    let app_name = "pp-finnone-structured-loans-merger";
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for the Finnone Structured Loans Merger.");
    info!(
        log,
        "Total Finnone Structured Loans Merger Duration: {:?}", total_duration
    );
    println!(
        "Total Finnone Structured Loans  Merger Duration: {:?}",
        total_duration
    );
}

...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for OD.");
    info!(log, "Total Duration for OD: {:?}", total_duration);
    println!(
        "Total Duration for OD .cf file generator: {:?}",
        total_duration
    );
}

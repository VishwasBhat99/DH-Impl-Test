...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for dimension to dimension aggregation.");
    info!(
        log,
        "Total Duration for dimension to dimension aggregation: {:?}", total_duration
    );
    println!(
        "Total Duration for dimension to dimension aggregation.{:2?}",
        total_duration
    );
}

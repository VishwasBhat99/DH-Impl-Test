...
fn main() {
    ...
    let app_name = "usd-to-inr-rate-collector";
    ...
    extractor::process(config_param, &log, &diagnostics_log);
    ...
    info!(
        log,
        "Total Duration taken by USD to INR Rate Collector merger: {:?}", total_duration
    );
    println!(
        "Total Duration taken by USD to INR Rate Collector merger: {:?}",
        total_duration
    );
}

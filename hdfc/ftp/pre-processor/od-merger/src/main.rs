...
fn main() {
    ...
    merger::process(config_param, &log, &diagnostics_log);
    ...
    info!(
        log,
        "Total Duration taken by OD merger: {:?}", total_duration
    );
    println!("Total Duration taken by OD merger: {:?}", total_duration);
}

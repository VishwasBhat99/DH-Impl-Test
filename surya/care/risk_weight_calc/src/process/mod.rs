...
pub fn calc_rw(config_params: &ConfigurationParameters, logger: &Logger, _diag_logger: &Logger) {
    ...
    for account in file_rdr.iter() {
        ...
        let col_acc_no = match account.get_string_for_key(&acc_keys.account_number) {
            Ok(val) => val.to_string(),
            Err(_) => format!(""),
        };
        if let Some(col_data) = col_data_map.get(&col_acc_no) {
...
fn get_iis_amt (
    ...
    let bal = match account.get_f64_for_key(&acc_keys.iis_amt) {
        Ok(val) => val,
        Err(_) => {
            let def = "0.0".to_string();
            let amt = account.get_string_for_key(&acc_keys.iis_amt).unwrap_or(&def);
            amt.parse().unwrap_or(0.0)
        }
    };
    let conv_bal = currency_converter.convert(&account, &currency, bal, config_params.is_consolidated(), logger);
    if config_params.is_consolidated() {
        bal
    } else {
        conv_bal

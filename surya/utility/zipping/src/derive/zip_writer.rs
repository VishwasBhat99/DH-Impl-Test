...
pub fn get_zip(src_dir: &str, dst_file: &str, log: &Logger, _diag_log: &Logger) {
    for &method in [
    ...
    .iter()
    {
        ...
        match doit(src_dir, dst_file, method.unwrap(), log, _diag_log) {
    ...
    T: Write + Seek,
{
    ...
    for entry in it {
        ...
        if path.is_file() {
            info!(log, "adding file {:?} as {:?} ...", path, name);
            #[allow(deprecated)]
            ...
            info!(log, "adding dir {:?} as {:?} ...", path, name);
            #[allow(deprecated)]

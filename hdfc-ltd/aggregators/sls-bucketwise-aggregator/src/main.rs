...
fn main() {
    ...
    let total_duration = end_generate_timer
        ...
        .expect("Could not calculate total duration for sls aggregation.");
    info!(
        log,
        "Total duration for sls Bucket wise Aggregation `{:?}`.", total_duration
    );
    println!(
        "Total Duration for sls Bucket wise Aggregation: {:2?}",
        total_duration
    );
}

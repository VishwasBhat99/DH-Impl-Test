...
fn main() {
    ...
    let app_name = "finnone-struct-loan-pp";
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate Total Finnone Structured Loans Pre-Processor duration.");
    info!(
        log,
        "Total Time Taken By Finnone Structured Loans Pre-Processor: {:?}", total_duration
    );
    println!(
        "Total Time Taken By Finnone Structured Loans Pre-Processor: {:#?}",
        total_duration
    );
}

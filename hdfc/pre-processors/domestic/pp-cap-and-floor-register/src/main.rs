...
fn main() {
    ...
    let app_name = "cap_floor_register";
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for cap_floor Register.");
    info!(
        log,
        "Total Duration for cap_floor Register: {:?}", total_duration
    );
    println!(
        "Total Duration for cap_floor Register cashflow file generator: {:2?}",
        total_duration
    );
}

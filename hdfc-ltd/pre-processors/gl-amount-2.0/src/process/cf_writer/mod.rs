    ...
    pub fn write(&mut self, record: Account) {
        ...
        let output_bytes = match record.write_length_delimited_to_bytes() {
            ...
            Err(e) => {
                log_error!(
                    self.log,
                    "Couldn't parse to bytes for `gl_code` : `{}`. Error: {:?}.",
                    record.gl_code,
                    e
                );
                return;
        ...
        if write_result.is_err() {
            panic!(
                "Couldn't write output bytes for `gl_code` : {}. Error: {:?}",
                record.gl_code,
                write_result
                    .err()
                    .expect("Unexpected error occured while writing output bytes for `gl_code`.")
            );
        }
    }
}

...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for CCIRS Register.");
    info!(
        log,
        "Total Duration for CCIRS Register: {:?}", total_duration
    );
    println!(
        "Total Duration for CCIRS Register cashflow file generator: {:2?}",
        total_duration
    );
}

...
fn main() {
    ...
    let app_name = "bullion-loans-cf-1.1.1";
    ...
    info!(
        log,
        "Total duration for Bullion Loans cashflow generation: `{:?}`.", total_duration
    );
    println!(
        "Total Duration for Bullion Loans cashflow generation: {:2?}",
        total_duration
    );
}

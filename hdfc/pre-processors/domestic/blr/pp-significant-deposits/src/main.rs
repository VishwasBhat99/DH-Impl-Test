...
fn main() {
    ...
    let app_name = "Depositors Pre-Processor";
    ...
    info!(diagnostics_log, "Start of processing deposits data.");
    processing::process_data(&config_param, &log, &diagnostics_log);
    info!(diagnostics_log, "End of processing deposits data.");
    ...
    info!(
        log,
        "Total Duration taken for deposits pre-processor: {:?}", total_duration
    );
    println!(
        "Total Duration taken for deposits pre-processor: {:?}",
        total_duration
    );
}

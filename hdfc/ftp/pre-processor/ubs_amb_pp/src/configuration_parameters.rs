...
impl ConfigurationParameters {
    pub fn log_parameters(&self, logger: &Logger) {
        ...
        info!(logger, "Input File Path: {}", self.input_file_path());
        info!(logger, "Exclude GL File Path: {}", self.ex_gl_file_path());
        info!(logger, "NPA account file path: {}", self.npa_file_path());
        info!(logger, "output_file_fixed: {}", self.output_file_path);
...
impl ConfigurationParameters {
    fn new_from_matches(matches: clap::ArgMatches) -> ConfigurationParameters {
        ...
        let ex_gl_file_path = matches
            .value_of("ex_gl_file_path")
            .expect("Error getting `ex_gl_file_path`.")
            .to_string();
...
impl ConfigurationParameters {
    ...
    pub fn ex_gl_file_path(&self) -> &str {
        &self.ex_gl_file_path
    }
...
fn get_eligible_arguments_for_app(app_name: &str) -> clap::ArgMatches {
    App::new(app_name)
        .about("AMB Data preprocessor for UBS")
        ...
        .arg(
            Arg::with_name("ex_gl_file_path")
                .long("gl-file")
                .value_name("Exclude GL File")
                .help("Path to the exclude gl file.")
                .required(true)
        )
        .arg(

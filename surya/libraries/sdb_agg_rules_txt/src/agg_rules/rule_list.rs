...
impl RSList {
    ...
    pub(crate) fn does_acc_abide(&self, account: &String, reader: &Reader) -> RSListAbidanceRes {
        for step in &self.steps {
            if step.does_acc_abide(account, reader) == true {
                continue;
            } else {
...
impl RS {
    pub fn does_acc_abide(&self, account: &String, reader: &Reader) -> bool {
        self.step.does_acc_abide(account, reader)
    }
}

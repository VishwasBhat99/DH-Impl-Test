    ...
    for account in account_reader.iter() {
        ...
        if config_params.is_consolidated() {
            ...
            if llg.currency != config_params.consolidated_currency() {
                ...
                let multiplier = exchange_rate.get_mut(&key_for_exchange_rate);
                if let Some(mult) = multiplier {
                    aggr_data.values_divided_by(1.0 / (*mult));
                }
                insert_into_store(&mut store, &llg, &aggr_data);
                let fcy_llg = LLGKey::new(
                    config_params.foreign_consolidation_currency().to_string(),

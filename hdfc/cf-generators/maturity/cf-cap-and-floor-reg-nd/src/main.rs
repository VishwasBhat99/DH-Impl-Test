...
fn main() {
    ...
    let app_name = "cap_floor_nd_register";
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for cap_floor_nd Register.");
    info!(
        log,
        "Total Duration for cap_floor_nd Register: {:?}", total_duration
    );
    println!(
        "Total Duration for cap_floor_nd Register cashflow file generator: {:2?}",
        total_duration
    );
}

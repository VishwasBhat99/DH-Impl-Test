...
impl<'a> InputAccount {
    pub fn new_from_line(
        ...
        let input_account = InputAccount {
            ...
            cntrprty_ctgry_5: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `cntrprty_ctgry_5`.");
                }
            },
            ind_outside_ind: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `ind_outside_ind`.");
                }
            },

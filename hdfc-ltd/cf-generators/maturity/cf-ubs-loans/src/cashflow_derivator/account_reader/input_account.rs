...
pub struct InputAccount {
    ...
    pub pre_emi_ovd_gl_cd: i32,
    pub exc_pre_emi_gl_cd: i32,
    pub emi_ovd_gl_cd: i32,
    pub exc_emi_gl_cd: i32,
    pub security: String,
    pub ncd_type: String,
...
impl<'a> InputAccount {
    pub fn new_from_line(
        ...
        let input_account = InputAccount {
            ...
            pre_emi_ovd_amt: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `pre_emi_ovd_amt`.");
                }
            },
            exc_pre_emi_amt: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `exc_pre_emi_amt`.");
                }
            },
            emi_ovd_amt: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `emi_ovd_amt`.");
                }
            },
            exc_emi_amt: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `exc_emi_amt`.");
                }
            },
            pre_emi_ovd_gl_cd: match value_iterator.next() {
                Some(val) => val.parse().unwrap_or(0),
                None => {
                    return Err("Could not parse property `pre_emi_ovd_gl_cd`.");
                }
            },
            exc_pre_emi_gl_cd: match value_iterator.next() {
                Some(val) => val.parse().unwrap_or(0),
                None => {
                    return Err("Could not parse property `exc_pre_emi_gl_cd`.");
                }
            },
            emi_ovd_gl_cd: match value_iterator.next() {
                Some(val) => val.parse().unwrap_or(0),
                None => {
                    return Err("Could not parse property `emi_ovd_gl_cd`.");
                }
            },
            exc_emi_gl_cd: match value_iterator.next() {
                Some(val) => val.parse().unwrap_or(0),
                None => {
                    return Err("Could not parse property `exc_emi_gl_cd`.");
                }
            },
            ...
            ncd_type: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `ncd_type`.");
                }
            },
            holder_type: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `holder_type`.");
                }
            },
        };
        Ok(input_account)

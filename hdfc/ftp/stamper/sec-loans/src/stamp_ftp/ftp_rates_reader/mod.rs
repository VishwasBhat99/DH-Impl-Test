...
pub fn read_ftp_rates(
    ...
    if Path::new(&ftp_rates_file_path).exists() {
        ...
        for line in rdr.lines() {
            ...
            let mut lst_rates: Vec<f64> = Vec::with_capacity(17);

            for i in 2..17 {
                lst_rates.push(line_components[i].parse().unwrap_or(0.0));
            }
            lst_rates.push(line_components[1].parse().unwrap_or(0.0));
            lst_rates.push(line_components[17].parse().unwrap_or(0.0));

...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for FTP Loans.");
    info!(
        log,
        "Total duration for Borrowing Lending Loans cashflow generation: `{:?}`.", total_duration
    );
    println!(
        "Total Duration for Borrowing Lending cashflow generation: {:2?}",
        total_duration
    );
}

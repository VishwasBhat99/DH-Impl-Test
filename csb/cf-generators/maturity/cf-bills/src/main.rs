...
fn main() {
    ...
    let total_duration = end_generate_timer
        ...
        .expect("Could not calculate total duration for Bills.");
    info!(
        log,
        "Total duration for Bills cashflow generation: `{:?}`.", total_duration
    );
    println!(
        "Total Duration for Bills cashflow generation: {:2?}",
        total_duration
    );
}

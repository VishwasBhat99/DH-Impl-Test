...
fn main() {
    ...
    let total_duration = end_generate_timer
        ...
        .expect("Could not calculate total duration for CF-OD-GL.");
    info!(
        log,
        "Total duration for CF-OD-GL cashflow generation: `{:?}`.", total_duration
    );
    println!(
        "Total Duration for CF-OD-GL cashflow generation: {:2?}",
        total_duration
    );
}

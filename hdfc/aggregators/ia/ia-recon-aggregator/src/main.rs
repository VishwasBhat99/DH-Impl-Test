...
fn main() {
    ...
    aggregator::aggregate(config_param, &log, &diagnostics_log);
    ...
    info!(
        log,
        "Total Duration taken by IA Recon Aggregator pre-processor: {:?}", total_duration
    );
    println!(
        "Total Duration taken by IA Recon Aggregator pre-processor: {:?}",
        total_duration
    );
}

...
use aggregator::duration_extensions::DurationAsNano;
...
pub fn new_writer_thread(
    ...
    thread_builder.spawn(move|| {
        ...
        let mut amount_totals_per_llg_report: HashMap<
            LLGKey,
            ...
            match aggregate_recv.recv() {
                Ok(aggregates_record) => {
                    ...
                    let report = log_measurements!(
                        diag_logger,
                        [format!("Type: AggregateToString, Identifier: {:?}", aggregates_record.prefix)],
                        {
                            append_to_string(
                    ...
                    build_report(
                        &mut amount_totals_per_llg_report,
                        report
                    );

                    log_measurements!(
                        diag_logger,
                        [format!("Type: WriteAggregate, Identifier: {:?}", aggregates_record.prefix)],
                        {
                ...
                Err(_error) => {
                    ...
                    log_measurements!(
                        ...
                        wrtr.flush().expect("Unable to flush record writer contents")
                    );
                    closing_report_sender.send(
                        AggregateWriterThreadReport {
                            thread_no,
...
fn append_to_string(
    ...
    let c_int_string = aggregates_record.prefix.to_string(CashflowAggregatedType::Int);
    let c_slr_string = aggregates_record.prefix.to_string(CashflowAggregatedType::Slr);
    let c_irs_string = aggregates_record.prefix.to_string(CashflowAggregatedType::Irs);
...
fn build_report(
    ...
    let mut existing = report.entry(new.0.clone()).or_insert(

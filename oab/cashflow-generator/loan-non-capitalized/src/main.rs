...
fn main() {
    ...
    let app_name = "cf_loan_non_capitalized";
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for Loan Non Capitalized cashflow generation.");
    info!(
        log,
        "Total duration for Loan Non Capitalized cashflow generation: `{:?}`.", total_duration
    );
    println!(
        "Total duration for Loan Non Capitalized cashflow generation: {:2?}",
        total_duration
    );
}

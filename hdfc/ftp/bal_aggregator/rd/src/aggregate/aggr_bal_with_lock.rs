...
pub fn aggr_bal_with_lock(ftp_parameters: &mut FtpParameters) -> (String, String) {
    ...
    for account_input in ftp_parameters.input_data.iter() {
        ...
        let custid = match account_input.get_i64_for_key(&inputfields.cust_id) {
            Ok(result) => result,
            Err(e) => DEFAULT_INT,
        };

        let ccy = match account_input.get_string_for_key(&inputfields.ccy) {
            Ok(result) => result,
            Err(e) => "",
        }
        .to_string();

        let start_dt = match account_input.get_i64_for_key(&inputfields.st_dt) {
            Ok(result) => result,
            Err(e) => DEFAULT_INT,
        };

        let mat_dt = match account_input.get_i64_for_key(&inputfields.mat_dt) {
            Ok(result) => result,
            Err(e) => DEFAULT_INT,
        };

        let amount = match account_input.get_f64_for_key(&inputfields.amt) {
            Ok(result) => result,
            Err(e) => DEFAULT_FLOAT,
        };

        let st_dt = NaiveDateTime::from_timestamp(start_dt, 0).date();

        total_bal_in += amount;

        if st_dt >= *ftp_parameters.cp.from_date() {

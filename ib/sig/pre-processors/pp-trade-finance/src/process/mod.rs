...
pub fn process(config_params: &ConfigurationParameters, logger: &Logger, diag_logger: &Logger) {
    let alm_trade_finance_reader = fs::read_to_string(&config_params.alm_trade_finance_file())
        .expect("Could Not Read ALM Trade Finance File");
    let mut op_writer = get_writer(&config_params.trade_finance_pp_file_path());
    ...
    for line in alm_trade_finance_reader.lines().skip(1) {
        let derived_fields = line.split("|").collect::<Vec<&str>>();
        tot_acc_encntrd += 1;

        let date_parser1 = DateParser::new("%Y-%m-%d".to_string(), false);
        let concat = format!("{}{}{}{}", derived_fields[11], derived_fields[2], derived_fields[8], derived_fields[9]);
        write!(
            op_writer,
            "{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}\n",
            ...
            derived_fields[3],
            derived_fields[4],
            date_parser1.parse_opt(&derived_fields[5]).unwrap_or(*config_params.as_on_date()).format("%d-%m-%Y"),
            date_parser1.parse_opt(&derived_fields[6]).unwrap_or(*config_params.as_on_date()).format("%d-%m-%Y"),
            derived_fields[7],
            derived_fields[8],
            derived_fields[9],
            derived_fields[10],
    ...
    health_report.gen_health_rpt(&config_params.trade_finance_pp_file_path());
} 
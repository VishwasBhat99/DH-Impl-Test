...
pub fn derive(config_params: ConfigurationParameters, log: &Logger, diag_log: &Logger) {
    ...
    loop {
        ...
        if input_account.buy_currency.to_uppercase() == "IN1"
            || input_account.sell_currency.to_uppercase() == "IN1"
        {
            skpd_acc += 1;
            log_error!(
                log,
                "Trade Id: `{}` exluded due to the currency fields: `{}` and `{}`.",
                input_account.trade_id,
                input_account.buy_currency,
                input_account.sell_currency,
            );
            continue;
        }
        let pay_leg_amt: f64 = input_account.buy_current_notional;
        let rec_leg_amt: f64 = input_account.sell_current_notional;
        let mut ccy = input_account.buy_currency.to_string();
        let mut cf_type = "I";

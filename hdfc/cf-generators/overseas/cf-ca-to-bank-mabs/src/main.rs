...
fn main() {
    ...
    let app_name = "cf-ca-to-bank-mabs";
    ...
    account_reader_writer::generate(
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for UBS CASA OD.");
    info!(log, "Total Duration for UBS CASA OD: {:?}", total_duration);
    println!(
        "Total Duration for UBS CASA OD .cf file generator: {:2?}",
        total_duration
    );
}

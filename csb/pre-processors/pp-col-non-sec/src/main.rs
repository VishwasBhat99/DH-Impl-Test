...
fn main() {
    ...
    let app_name = "collateral-non-security-pp";
    ...
    info!(
        log,
        "Total Duration taken by Collateral Non-Security Data pre-processor: {:?}", total_duration
    );
    println!(
        "Total Duration taken by Collateral Non-Security Data pre-processor: {:?}",
        total_duration
    );
}

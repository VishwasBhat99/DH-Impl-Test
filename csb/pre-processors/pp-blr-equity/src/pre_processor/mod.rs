...
fn get_exchange_rates(exchange_rate_path: &str, log: &Logger) -> HashMap<NaiveDate, f64> {
    ...
    let mut prev: f64 = 0f64;
    let mut exchange_rate_file: Xlsx<_> =
        open_workbook(exchange_rate_path).expect("Unable to open `exchange_rate_file`.");
    let sheet1 = exchange_rate_file
        .sheet_names()
        .first()
        .expect("excel is empty")
        .to_owned();
    if let Some(Ok(reader)) = exchange_rate_file.worksheet_range(sheet1.as_str()) {
        for row in reader.rows() {
            match rbdate::datevalue_to_naive_date(row[0].to_string().as_str()) {
                Ok(date) => {
                    let rate = str_to_float(row[1].to_string().as_str(), prev);
                    prev = rate;
                    exchanges_rates.insert(date, rate);
                }
                Err(err) => {
                    log_error!(log, "{}", err);
                }
...
fn str_to_float(num: &str, prev: f64) -> f64 {
    num.parse::<f64>().unwrap_or(prev)
}

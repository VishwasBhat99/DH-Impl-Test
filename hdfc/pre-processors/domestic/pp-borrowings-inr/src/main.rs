...
fn main() {
    ...
    let total_duration = end_main_timer.duration_since(start_main_timer).expect(
        "Could not calculate total duration for Borrowings and Lendings Pre-processor INR.",
    );
    info!(
        log,
        "Total Duration for Borrowings and Lendings Pre-processor INR: {:?}", total_duration
    );
    println!(
        "Total Duration for Borrowings and Lendings Pre-processor INR: {:2?}",
        total_duration
    );
}

...
pub fn append_cf_data(
    out_acc: &mut Account,
    account_data: &InputAccount,
    _ason: NaiveDate,
    rep_data_map: &HashMap<String, NaiveDate>,
    config_params: &ConfigurationParameters,
) {
    let acct_id = account_data.key_1.clone();
    let mut last_six_digits = "".to_string();
    if acct_id.len() >= 6 {
        last_six_digits = acct_id[acct_id.len() - 6..].to_owned();
    }
    let mut next_rep_date = rbdate::NaiveDate::parse_from_str("31-12-1899", "%d-%m-%Y")
        .unwrap_or(*config_params.as_on_date());
    if rep_data_map.contains_key(last_six_digits.trim()) {
        next_rep_date = rep_data_map
            .get(last_six_digits.trim())
            .unwrap_or(config_params.as_on_date())
            .clone();
    }
    ...
    out_acc.a6 = account_data.a6.to_string();
    ...
    out_acc.next_rep_date = timestamp(next_rep_date);
}

...
pub fn get_op_line(
    ...
    loans.push(rec[1].to_string() + "|" + &alm_concat + "|" + alm_line + "\n");
    loans.push(get_line(rec, as_on_dt, div, alm_line, log));
    loans
}
...
fn get_line(
    ...
    let rt_chng_freq: f64 = if let (Ok(st_dt), Ok(end_dt)) = (
        NaiveDate::parse_from_str(val[25].as_str(), "%d-%m-%Y"),
        NaiveDate::parse_from_str(val[26].as_str(), "%d-%m-%Y"),
    ) {
        if st_dt < end_dt {
...
fn get_date(val: &Vec<String>, row_num: usize, name: &str, log: &Logger) -> String {
    let error_msg = format!(
        "Unable to parse `{}: `{}` in `DD-MM-YYYY` for account: `{}`:",

...
pub struct InputAccount {
    ...
    pub inr_amt: f64,
    pub inr_rt: f64,
    pub contract_usage: String,
    pub non_index_flow_currency: String,
    pub non_index_cf_amt: f64,
    pub indexation_dt: Option<NaiveDate>,
}
...
impl<'a> InputAccount {
    pub fn new_from_line(
        ...
        let input_account = InputAccount {
            ...
            contract_usage: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `contract_usage`.");
                }
            },
            non_index_flow_currency: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `non_index_flow_currency`.");
                }
            },
            non_index_cf_amt: match value_iterator.next() {
                Some(val) => val.parse().unwrap_or(DEFAULT_FLOAT),
                None => {
                    return Err("Could not parse property `non_index_cf_amt`.");
                }
            },
            indexation_dt: match value_iterator.next() {
                Some(val) => dmy_date_parser.parse_opt(val),
                None => {
                    return Err("Could not parse property `indexation_dt`.");
                }
            },
        };
        Ok(input_account)

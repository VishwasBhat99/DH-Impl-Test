...
pub struct InputAccount {
    ...
    pub int_type: String,
    pub int_rt: f64,
    pub next_rep_dt: Option<NaiveDate>,
    pub last_rep_dt: Option<NaiveDate>,
    pub rep_freq: String,
    pub benchmark: String,
    pub cust_class: String,
    pub npa_class: String,
    pub ason: Option<NaiveDate>,
    ...
    pub class2: String,
    pub class3: String,
    pub actual_cust_care_value: String,
    pub actual_acc_care_value: String,
    pub int_benchmark: String,
}
...
impl<'a> InputAccount {
    pub fn new_from_line(
        ...
        let input_account = InputAccount {
            ...
            lbm_gl: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `lbm_gl`.");
                }
            },
            ...
            bal_lcy: match value_iterator.next() {
                Some(val) => val.parse().unwrap_or(DEFAULT_FLOAT),
                None => {
                    return Err("Could not parse property `bal_lcy`.");
                }
            },
            ...
            next_rep_dt: match value_iterator.next() {
                Some(val) => dmy_date_parser.parse_opt(val),
                None => {
                    return Err("Could not parse property `next_rep_dt`.");
                }
            },
            last_rep_dt: match value_iterator.next() {
                Some(val) => dmy_date_parser.parse_opt(val),
                None => {
                    return Err("Could not parse property `last_rep_dt`.");
                }
            },
            ...
            cust_class: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `cust_class`.");
                }
            },
            ...
            ason: match value_iterator.next() {
                Some(val) => dmy_date_parser.parse_opt(val),
                None => {
                    return Err("Could not parse property `ason`.");
                }
            },
            bank_name: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `bank_name`.");
                }
            },
            ...
            category_of_borrower: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
            ...
            class1: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `class1`.");
                }
            },
            class2: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `class2`.");
                }
            },
            class3: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `class3`.");
                }
            },
            actual_cust_care_value: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `actual_cust_care_value`.");
                }
            },
            actual_acc_care_value: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `actual_acc_care_value`.");
                }
            },
            int_benchmark: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `int_benchmark`.");
                }
            },
        };
        Ok(input_account)

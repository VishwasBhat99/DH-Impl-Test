...
pub fn generate(config_params: &ConfigurationParameters, log: &Logger, diag_log: &Logger) {
    ...
    loop {
        ...
        tot_prin_in_in += input_account.out_amt;
        let cashflows_for_account_result = log_measurements!(
            diag_log,
            [format!(
                "Type: GenCFs, Identifier: `{}`",
                input_account.acct_num
            )],
            generate_cashflows(&mut input_account)
        );
        ...
        let account_with_cashflows = log_measurements!(
            ...
            create_account_with_cashflows(input_account, cashflows.clone())
        );

        tot_prin_in_op += cashflows[0].get_principal_amount();
        tot_int_in_op += 0.0;
        log_measurements!(
            diag_log,
            [format!(
                "Type: WriteAccWithCFs, Identifier: {}",
                account_with_cashflows.acct_num
            )],

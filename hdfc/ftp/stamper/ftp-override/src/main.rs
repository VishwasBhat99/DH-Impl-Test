...
fn main() {
    ...
    process::override_output(config_param, &log, &diagnostics_log);
    let end_main_timer = SystemTime::now();
    let total_duration = end_main_timer
        .duration_since(start_main_timer)
        .expect("Could not calculate Total FTP Override Process duration.");
    info!(
        log,
        "Total Time Taken By FTP Override Process: {:?}", total_duration
    );
    println!(
        "Total Time Taken By FTP Override Process: {:#?}",
        total_duration
    );
}

...
impl ConfigurationParameters {
    pub fn log_parameters(&self, logger: &Logger) {
        ...
        info!(logger, "is_header: {}", self.is_header());
    }
}
...
impl ConfigurationParameters {
    fn new_from_matches(matches: clap::ArgMatches) -> ConfigurationParameters {
        ...
        let is_header = matches
            .value_of("is_header")
            .expect("Error getting `is_header` value.")
            .parse::<bool>()
            .expect("Cannot parse `is_header` value as bool.");
...
fn get_eligible_arguments_for_app(app_name: &str) -> clap::ArgMatches {
    App::new(app_name)
        ...
        .arg(
            Arg::with_name("is_header")
                .long("is-header")
                .value_name("Is Header")
                .possible_values(&["true", "false"])
                .help("This flag that decides whether the file contains header or not.")
                .default_value("true")
                .required(false)
        )
        .arg(

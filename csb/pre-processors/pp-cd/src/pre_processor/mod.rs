...
pub fn process(config_param: ConfigurationParameters, log: &Logger, diag_log: &Logger) {
    ...
    for (line_num, lines) in input_reader.deserialize().enumerate().skip(1) {
        ...
        let amt = input_account.os_bal_cry.parse().unwrap_or(DEFAULT_FLOAT);
        tot_amt += amt;
        ...
        let recon_key = ReconKey::new(input_account.ccy, String::from("CD"), input_account.gl);
        recon
            .store
            .entry(recon_key)
    ...
    output_writer(
        &mut recon_writer,
        recon.print(*config_param.as_on_date(), "CD"),
    ...
    debug!(diag_log, "Writing CD, Total Duration: {:?}.", duration);
}

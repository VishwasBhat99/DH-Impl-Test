...
use crate::cashflow_derivator::account_with_cashflows_writer::account_indices::Index;
use crate::macros;
...
impl AccountWithCashflowsWriter {
    pub fn new(path: &str, log: &Logger) -> AccountWithCashflowsWriter {
        AccountWithCashflowsWriter {
            account_w_cf_writer: sdb_io::buf_file_wrtr(&get_full_path(path, ".cf"), None)
                .expect("Unable to create `.cf` file."),
            account_indices_writer: sdb_io::buf_file_wrtr(&get_full_path(path, ".idx"), None)
    ...
    pub fn write(&mut self, record: AccountWithCashflows) {
        ...
        if write_result.is_err() {
            panic!(
                ...
                write_result
                    .err()
                    .expect("Unexpected error occured while wriring cf file.")
            );
        }

        // Write the index for this account:
        self.account_index.set_key(record.cust_no.to_string());
...
pub fn flush_contents(mut writer: BufWriter<File>, writer_name: &str) {
    ...
    if flush_result.is_err() {
        panic!(
            ...
            flush_result
                .err()
                .expect("Unexpected error occured while flushing buffer writer")
        );
    }
}

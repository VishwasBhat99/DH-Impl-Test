...
use self::structs::{
    alm_master::*, basel_passthroughs::BaselPassThroughs, cust_master::*,
...
pub fn process(config_param: ConfigurationParameters, log: &Logger, diag_log: &Logger) {
    ...
    let mut overdue_line: String = String::new();
    ...
    for (line_num, lines) in input_reader.deserialize().enumerate().skip(1) {
        ...
        if let Ok(over_dt) = NaiveDate::parse_from_str(&input_account.over_dt, "%d-%m-%Y") {
            if over_dt <= *config_param.as_on_date() {
                skp_rec += 1;
                log_warn!(log, "Overdue for account: `{}`.", input_account.acc_no);
                overdue_line.push_str(&get_op_line(&mut input_account,&mut cust_master,
                    &mut alm_master,
        ...
        let recon_key = ReconKey::new(input_account.ccy, String::from("TD"), input_account.gl_cd);
        recon
            .store
            .entry(recon_key)
    ...
    output_writer(
        &mut recon_writer,
        recon.print(*config_param.as_on_date(), "TD"),
    ...
    let mut overdue_acc_writer = get_writer(config_param.overdue_acc_file_path());
    output_writer(
        &mut overdue_acc_writer,
        overdue_line,
        config_param.overdue_acc_file_path(),
    );
    ...
    debug!(diag_log, "Writing TD, Total Duration: {:?}.", duration);
}

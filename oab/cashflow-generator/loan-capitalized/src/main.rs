...
fn main() {
    ...
    let app_name = "cf_loan_capitalized";
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for Loan Capitalized cashflow generation.");
    info!(
        log,
        "Total duration for Loan Capitalized cashflow generation: `{:?}`.", total_duration
    );
    println!(
        "Total duration for Loan Capitalized cashflow generation: {:2?}",
        total_duration
    );
}

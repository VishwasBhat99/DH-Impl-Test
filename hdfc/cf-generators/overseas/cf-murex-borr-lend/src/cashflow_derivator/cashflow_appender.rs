...
use cashflow_derivator::account_with_cashflows::money_market;
...
pub fn create_account_with_cashflows(
    ...
    let mut out_acc = money_market::new();
    ...
    out_acc.val_dt = if let Some(dt) = account.val_date {
        timestamp(dt)
    } else {
        DEFAULT_INT
    };
    out_acc.deal_dt = if let Some(dt) = account.deal_date {
        timestamp(dt)
    } else {
        DEFAULT_INT
    };
    ...
    out_acc.tenor_days = account.tenor_days.to_string();
    ...
    out_acc.cntrprty_cat_1 = account.cntrprty_ctgry_1;
    out_acc.cntrprty_cat_2 = account.cntrprty_ctgry_2;
    out_acc.cntrprty_cat_3 = account.cntrprty_ctgry_3;
    out_acc.cntrprty_cat_4 = account.cntrprty_ctgry_4;
    out_acc.int_pay_rec = account.int_pay_rec;
    out_acc.bckt_days = account.bckt_days;
    out_acc.sys_gl = account.system_gl;
    ...
    out_acc.balm_l2 = account.balm_l2;
    out_acc.funding_source = account.funding_source;
    for cf in &cashflows {

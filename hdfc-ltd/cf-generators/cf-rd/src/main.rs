...
fn main() {
    ...
    let total_duration = end_generate_timer
        ...
        .expect("Could not calculate total duration for Recurring Deposits.");
    info!(
        log,
        "Total duration for Recurring Deposits cashflow generation: `{:?}`.", total_duration
    );
    println!(
        "Total Duration for Recurring Deposits cashflow generation: {:2?}",
        total_duration
    );
}

...
pub fn process(config_param: ConfigurationParameters, log: &Logger, diag_log: &Logger) {
    ...
    if let Some(Ok(reader)) = input_file.worksheet_range(config_param.sheet_name()) {
        for mut row in reader.rows() {
            ...
            if row[2].to_string().to_uppercase() == "BORROWING" {
                ...
                let amt: f64 = row[9].to_string().parse().unwrap_or(0.0);
                recon
                    .entry(recon_key)
                ...
                tot_bor_acc += 1;
            } else if row[2].to_string().to_uppercase() == "LENDING" {
    ...
    debug!(
        diag_log,
        "Writing Accrued Borrowings & Lendings Records and Reconcilation File, Total Duration: {:?}.",
        duration
    );

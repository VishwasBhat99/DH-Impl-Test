...
fn main() {
    ...
    let total_duration = end_main_timer.duration_since(start_main_timer).expect(
        "Could not calculate total duration for Borrowings and Lendings Pre-processor FCY.",
    );
    info!(
        log,
        "Total Duration for Borrowings and Lendings Pre-processor FCY: {:?}", total_duration
    );
    println!(
        "Total Duration for Borrowings and Lendings Pre-processor FCY: {:?}",
        total_duration
    );
}

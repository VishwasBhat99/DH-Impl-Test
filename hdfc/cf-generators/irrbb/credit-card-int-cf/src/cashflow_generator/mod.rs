...
pub fn generate(
    ...
    loop {
        ...
        total_principal_in_input += input_account.amount;
        let cashflows_for_account_result =
            match (input_account.maturity_date, input_account.next_repr_date) {
                (Some(_mat_date), Some(_next_repr_date)) => {
                    log_measurements!(
                        diag_log,
                        [format!(
                        ...
                        generate_cashflows(as_on_date, &mut input_account, log)
                    )
                }
                _ => {
                    log_error!(
                        log,
                        "Next Reprice Date/ Maturity Date not present for : '{}'.",
                        input_account.account_number
                    );
                    continue;
                }
        ...
        // Continue processing cashflows_for_account_result outside the match
        ...
        for cf in cashflows.iter() {
            total_interest_in_output += cf.interest_amount;
            total_principal_in_output += cf.principal_amount;
        }

...
fn main() {
    ...
    let app_name = "securitisation-1.0.1";
    ...
    let total_duration = end_time
        ...
        .expect("Could not calculate total duration for Securitisation Pre-processor.");
    info!(
        log,
        "Total Duration taken by Securitisation Pre-processor: {:?}", total_duration
    );
    println!(
        "Total Duration taken by Securitisation Pre-processor: {:?}",
        total_duration
    );
}

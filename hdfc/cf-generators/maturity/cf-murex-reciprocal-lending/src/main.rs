...
fn main() {
    ...
    let app_name = "reciprocal_lending";
    ...
    cashflow_derivator::derive(config_params, &log);
    ...
    info!(
        log,
        "Total Duration of Reciprocal Lending Cashflow derivator: {:?}", total_duration
    );
    println!(
        "Total Duration of Reciprocal Lending Cashflow derivator: {:?}",
        total_duration
    );
}

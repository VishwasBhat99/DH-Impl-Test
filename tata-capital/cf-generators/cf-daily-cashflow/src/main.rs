...
fn main() {
    ...
    let app_name = "cf_daily_cashflow";
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not get daily cashflows.");
    info!(
        log,
        "Total Duration for getting daily cashflows: {:?}", total_duration
    );
    println!(
        "Total Duration for getting daily cashflows.{:2?}",
        total_duration
    );
}

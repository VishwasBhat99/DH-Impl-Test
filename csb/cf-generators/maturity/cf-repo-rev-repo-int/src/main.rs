...
fn main() {
    ...
    let app_name = "repo-rev-repo-cf-int";
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for Repo Rev Repo.");
    info!(
        log,
        "Total Duration for Repo Rev Repo: {:?}", total_duration
    );
    println!(
        "Total Duration for Repo Rev Repo .cf file generator: {:2?}",
        total_duration
    );
}

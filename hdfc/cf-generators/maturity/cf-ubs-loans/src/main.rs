...
fn main() {
    ...
    let app_name = "ubs-loans-cf-1.0.3280";
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for UBS Loans.");
    info!(
        log,
        "Total duration for UBS Loans cashflow generation: `{:?}`.", total_duration
    );
    println!(
        "Total Duration for UBS Loans cashflow generation: {:2?}",
        total_duration
    );
}

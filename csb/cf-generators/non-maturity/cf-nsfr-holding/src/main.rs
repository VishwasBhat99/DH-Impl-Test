...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for NPA.");
    info!(log, "Total Duration for NSFR ISI {:?}", total_duration);
    println!(
        "Total Duration for NSFR ISI .cf file generator: {:2?}",
        total_duration
    );
}

...
use pre_processor::structs::pass_throughs::PassThroughs;
...
pub fn get_op_line(
    ...
    acc.open_dt = match NaiveDate::parse_from_str(&acc.open_dt, "%d-%b-%y") {
        Ok(dt) => dt.format("%d-%m-%Y").to_string(),
        Err(error) => {
            log_error!(log,
                "`account_open_date`: `{}` is not well-formatted as `DD-MMM-YY` for account: `{}` : `{}`.", 
                acc.open_dt,
                acc.acc_no,error);
            String::default()
        }
    ...
    let pass_throughs = PassThroughs::new();
    ...
    op_line.push_str(&pass_throughs.print());
    op_line.push('\n');

    if alm_master.balm_llg == "NONE" {
        concats.push_str(&format!(
            "{}|{}|{}|{}\n",
            src_file_name, acc.acc_no, acc.gl, acc.os_bal
        ));
    }
    op_line
}

...
pub fn aggregate_cashflows(
    ...
    let summary_header = "Account Number|Source system|Customer Id|Customer Name|Product code|Scheme ID|Booking Date|Validity Date|Maturity Date|CCY|MIS 1|MIS 2|MIS 3|Source GL|Rate Type|Original Amount|Current o/s amount|Interest rate previous month|Interest rate current month|Interest Rate differential|ALM line|IA Line|Concat|Division|NPA Type|Raw BM|Final BM|Old Rate Flag|New Rate Flag\n";
    let drilldown_header = "Account Number|Source system|Customer Id|Customer Name|Product code|Scheme ID|Booking Date|Validity Date|Maturity Date|CCY|MIS 1|MIS 2|MIS 3|Source GL|Rate Type|Original Amount|Current o/s amount|Interest rate previous month|Interest rate current month|Interest Rate differential|Opening Balance|Principal|Interest Amt|Reducing Principal|FTM impact|Residual Tenor|Residual Tenor (in Years)|Residual Tenor Impact|Discount Factor|Present Value|ALM line|IA Line|Concat|Division|NPA Type|Raw BM|Final BM|Old Rate Flag|New Rate Flag|Cash Flow Date\n";
    ...
    for file in files_config.input_files {
        ...
        let rate_type_value = file.rate_type_value;
        for mut account in cur_mth_reader.iter() {
            acc_enc += 1;
            let rate_type = account
                .get_string_for_key(&keys.rt_type)
                .unwrap_or(&"NA".to_string())
                .to_string();
            //Process the account only if rate type is Fixed.
            if rate_type == rate_type_value {
                let acc_no = account
                    .get_string_for_key(&keys.acc_no)
                    .unwrap_or(&"NA".to_string())
                    .to_string();
                ...
                if let Some(data) = prev_mth_map.get(&acc_no) {
                    ...
                    //Report account only if interest rate has changed.
                    if int_diff != 0.00 {
                        acc_succ += 1;
                        ...
                        let booking_dt =
                            match account.get_i64_for_key(&keys.booking_dt).unwrap_or(0) {
                                0 => "NA".to_string(),
                                dt => naivedate_from_timestamp(dt).format("%d-%m-%Y").to_string(),
                            };
                        let validity_dt =
                            match account.get_i64_for_key(&keys.validity_dt).unwrap_or(0) {
                                0 => "NA".to_string(),
                                dt => naivedate_from_timestamp(dt).format("%d-%m-%Y").to_string(),
                            };
                        ...
                        let org_amt = account.get_f64_for_key(&keys.org_amt).unwrap_or(0.00);
                        ...
                        let leading_fields = OpLeadingFields {
                            ...
                            int_diff: int_diff.to_owned().to_string(),
                        };
                        ...
                        let old_rt_flag = &data.rate_flag;
                        ...
                        let mut opening_bal = os_amt;
                        //initialize prev cf date as None.
                        ...
                        for cf in cashflows.iter_mut() {
                            ...
                            let red_principal = opening_bal - (prin_amount + int_amount);
                            ...
                            //FTM impact
                            let ftm_impact =
                                os_amt * int_diff / 100.0 * (no_of_days as f64) / 365.0;

                            //Residual Tenor
                            let res_tenure = num_days_start_to_end(as_on_date, cf_date);
                            //Residual tenor in years
                            //TODO: To be replaced with convention logic later.
                            let res_ten_yr = res_tenure as f64 / 365.0;
                            //Residual tenor impact
                            let res_ten_impact =
                                prin_amount * int_diff / 100.0 * (res_tenure as f64) / 365.0;
                            //Discounted Factor
                            ...
                            let derived_fields_op = OpDrilldownReport {
                                opening_bal,
                                principal: prin_amount,
                                interest_amt: int_amount,
                                reducing_principal: red_principal,
                                ftm_impact,
                                residual_tenor: res_tenure,
                                residual_tenor_yrs: res_ten_yr,

...
pub fn get_processed_data_ei(
    reader_map: &mut MultiMap<InputKeyEI, InputDataEI>,
    writer_map: &mut HashMap<OPKeyEI, OPDataEI>,
            ...
            for val in data.clone() {
                if val.flow_start_date >= *config_params.as_on_date()
                    || val.flow_start_date
                        == data
                            .iter()
                            .filter(|d| d.flow_start_date < *config_params.as_on_date())

...
fn main() {
    ...
    let app_name = "avg-bal-close-aggr";
    ...
    aggregator::process(config_param, &log, &diagnostics_log);
    let end_main_timer = SystemTime::now();
    let total_duration = end_main_timer
        .duration_since(start_main_timer)
        .expect("Could not calculate total duration for Average Balance Aggregator.");
    info!(
        log,
        "Total Duration for Average Balance Aggregator: {:?}", total_duration
    );
    println!(
        "Total Duration for Average Balance Aggregator: {:.2?}",
        total_duration
    );
}

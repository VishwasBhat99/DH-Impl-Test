    ...
    loop {
        ...
        if account_opt.is_none() {
            ...
            if sum_prin_acc != a_w_cf.outstanding_bal {
                ...
                generate_adjustment_cashflow(&a_w_cf, &mut sum_prin_acc, &mut cfs, log);
            }
        ...
        if input_account.field_type == "I" {
            prin_amt = DEFAULT_FLOAT;
            int_amt += input_account.cf_principal_amount;
        } else if input_account.field_type == "P" {
            prin_amt = input_account.cf_principal_amount - int_amt;
        ...
        let cashflows = log_measurements!(
            ...
            if input_account.field_type == "I" {
                derive_cashflows(
                    &mut input_account.clone(),
                    input_account.cf_principal_amount,
                    DEFAULT_FLOAT,
                    &config_params,
                    log,
                )
                ...
                log_info!(
                    log,
                    "field_type is neither I nor P for account : {}.",
                    input_account.account_id.to_string()
                );
                derive_cashflows(
                    &mut input_account,
                    DEFAULT_FLOAT,

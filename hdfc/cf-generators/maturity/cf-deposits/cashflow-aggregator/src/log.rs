...
pub fn setup_loggers(log_file_path: &str, diagnostics_file_path: &str) -> (slog::Logger, slog::Logger) {
    ...
    let logger_drain = drain_builder.overflow_strategy(OverflowStrategy::Block).build().fuse();
    ...
    let diag_drain = diag_drain_builder.use_custom_timestamp(local_timestamp_utc).build().fuse();

    let async_diag_drain_builder = slog_async::Async::new(diag_drain);
    let async_diag_drain = async_diag_drain_builder.overflow_strategy(OverflowStrategy::Block).build().fuse();

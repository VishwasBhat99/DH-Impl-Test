...
impl ConfigurationParameters {
    pub fn log_parameters(&self, logger: &Logger) -> () {
        ...
        info!(logger, "bkt_scheme: {}", self.bkt_scheme());
        info!(
            logger,
            "org_tenor_file_path: {}",
            self.org_tenor_file_path()
        );
        info!(logger, "bkt_scheme_id: {}", self.bkt_scheme_id());
...
impl ConfigurationParameters {
    fn new_from_matches(matches: clap::ArgMatches) -> ConfigurationParameters {
        ...
        let bkt_scheme = matches.value_of("bkt_scheme").unwrap().to_string();
        let org_tenor_file_path = matches.value_of("org_tenor_file_path").unwrap().to_string();
        let bkt_scheme_id = matches.value_of("bkt_scheme_id").unwrap().to_string();
...
impl ConfigurationParameters {
    ...
    pub fn bkt_scheme(&self) -> &str {
        return &self.bkt_scheme;
    }
    pub fn org_tenor_file_path(&self) -> &str {
        return &self.org_tenor_file_path;
    }
    pub fn bkt_scheme_id(&self) -> &str {
        return &self.bkt_scheme_id;
    }
}
...
fn get_eligible_arguments_for_app(app_name: &str) -> clap::ArgMatches {
    App::new(app_name)
        ...
        .arg(
            Arg::with_name("bkt_scheme")
                .short("bs")
                .long("bkt-scheme")
                .value_name("BUCKET SCHEME")
                .help("Bucket Scheme")
                .required(true),
        )
        .arg(

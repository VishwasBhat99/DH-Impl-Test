...
pub struct InputAccount {
    ...
    pub division: String,
    pub lcr_class_code: String,
    pub contr_mat: String,
    pub res_mat: String,
...
impl InputAccount {
    pub fn new_from_line(
        ...
        let input_account = InputAccount {
            ...
            lcr_class_code: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not read property `lcr_class_code`.".to_string());
                }
            },
            contr_mat: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not read property `contr_mat`.".to_string());
                }
            },
            res_mat: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not read property `res_mat`.".to_string());
                }
            },
            bal_int_comp_lcy: match value_iterator.next() {
                Some(val) => val.parse().unwrap_or(DEFAULT_FLOAT),
                None => {
                    return Err("Could not read property `bal_int_comp_lcy`.".to_string());
                }
            },
        };

...
fn main() {
    ...
    let app_name = "murex_borrowings_and_lendings";
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for Borrowings and Lendings Pre-processor.");
    info!(
        log,
        "Total Duration for Borrowings and Lendings Pre-processor: {:?}", total_duration
    );
    println!(
        "Total Duration for Borrowings and Lendings Pre-processor: {:?}",
        total_duration
    );
}

...
use super::currency::currency_converter::CurrencyConverter;
...
pub fn get_dim(
    ...
    match dim_type {
        "NUMSLAB" => {
            let key = &fields[0];
            let mut value = match get_field_value(&account, &method_reader, key.to_string()) {
                Ok(value) => value.to_string().parse::<f64>().unwrap_or(0.0),
                Err(_error) => panic!("{}", _error),
            };
            if is_numslab_consol_req {
                value = currency_converter.convert_numslab(
                    ccy,
                    value,
                    is_consolidated,
        ...
        "PRDSLAB" => {
            ...
            let mut value = 0.0;
            //if dates are I64
            if is_string_numeric(&st_dt) && is_string_numeric(&end_dt) {
                let st_dt = naivedate_from_timestamp(st_dt.to_string().parse::<i64>().unwrap_or(0));
                let end_dt =
                    naivedate_from_timestamp(end_dt.to_string().parse::<i64>().unwrap_or(0));
                value = get_value(st_dt, end_dt);
            }
            //if dates are String
            else {
                let st_dt = NaiveDate::parse_from_str(&st_dt.to_string(), "%d-%m-%Y").unwrap();
                let end_dt = NaiveDate::parse_from_str(&end_dt.to_string(), "%d-%m-%Y").unwrap();
                value = get_value(st_dt, end_dt);
            }
...
pub fn get_prd_slabs(period_slab_vec: &Vec<Vec<String>>, as_on_date: &NaiveDate) -> Vec<RangeSlab> {
    let mut slabs: Vec<RangeSlab> = Vec::new();

    for info in period_slab_vec {
...
fn get_value(st_dt: NaiveDate, end_dt: NaiveDate) -> f64 {

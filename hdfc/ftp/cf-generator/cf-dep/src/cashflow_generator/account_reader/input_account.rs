...
pub struct InputAccount {
    ...
    pub date_last_withdraw: NaiveDate,
    pub prepayment_less_than_7_days: String,
    pub two_point_concat: String,
    pub four_point_concat: String,
}
...
impl InputAccount {
    pub fn new_from_line(
        ...
        let input_account = InputAccount {
            ...
            date_last_withdraw: match value_iterator.next() {
                Some(val) => {
                    let date_last_withdraw = dmy_date_parser.parse_opt(val);
                    if date_last_withdraw.is_none() {
                        return Err("Could not parse property `date_last_withdraw`.".to_string());
                    }
                    date_last_withdraw.unwrap()
                }
                None => {
                    return Err("Could not read property `date_last_withdraw`.".to_string());
                }
            },
            prepayment_less_than_7_days: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err(
                        "Could not read property `prepayment_less_than_7_days`.".to_string()
                    );
                }
            },
            two_point_concat: match value_iterator.next() {
                ...
                None => {
                    return Err(
                        "Could not read property `two_point_concat`.".to_string()
                    );
                }
            },
            four_point_concat: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err(
                        "Could not read property `four_point_concat`.".to_string()
                    );
                }
            },
        };

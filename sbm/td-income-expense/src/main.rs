...
fn main() {
    ...
    let app_name = "td-income-expense-generator";
    ...
    generator::generate(config_param, &log, &diagnostics_log);
    let end_main_timer = SystemTime::now();
    let total_duration = end_main_timer
        .duration_since(start_main_timer)
        .expect("Could not calculate total duration for TD Income Expense Generator.");
    info!(
        log,
        "Total Duration for TD Income Expense Generator: {:?}", total_duration
    );
    println!(
        "Total Duration for TD Income Expense Generator: {:.2?}",
        total_duration
    );
}

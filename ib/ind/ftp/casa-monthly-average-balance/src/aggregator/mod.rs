...
pub fn generatesummary(
    ...
    for line in reader.lines() {
        ...
        let mut outbal_sum = derived_fields[1].to_string().parse::<f64>().unwrap_or(0.0);
        let mut int_rate = derived_fields[2].to_string().parse::<f64>().unwrap_or(0.0);
        let mut sum_prod_outbal_intrt = derived_fields[5].to_string().parse::<f64>().unwrap_or(0.0);
        let mut div_posted_sum = derived_fields[3].to_string().parse::<f64>().unwrap_or(0.0);
        let div_posted_end_date = derived_fields[4].to_string().parse::<f64>().unwrap_or(0.0);
        let mut method_id = String::new();
        let gl_cd = derived_fields[9];
        let curr_status = derived_fields[6].to_string();
        let closed_dt = NaiveDate::parse_from_str(derived_fields[8], "%d-%m-%Y").unwrap_or(default_date);
        let class = derived_fields[7];
        ...
        let int_posted = div_posted_end_date;

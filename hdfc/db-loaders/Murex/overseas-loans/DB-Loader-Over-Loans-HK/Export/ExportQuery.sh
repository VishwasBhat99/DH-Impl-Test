...
echo "Start of deleting the old over-loans-hk.txt file"

rm /data/oracle18c/app/product/18c/dbhome/ExportLog/over-loans-hk.txt

echo "End of deleting the old over-loans-hk.txt file"

echo "Start of spooling the data"

sqlplus -s balmusr/HdFcBank13\$\# << ENDOFSQL
set linesize 2000 trims on  pagesize 0 feedback off;
SPOOL /data/oracle18c/app/product/18c/dbhome/ExportLog/over-loans-hk.txt;
select A1.CUSTOMER
||'|'||A1.REFERENCE
||'|'||A1.CUSTOMER_NAME
||'|1|'||A1.NORM_INT_RATE
...
||'|'||A1.LAST_RESET_DATE
||'|'||A1.NEXT_RESET_DATE
from OVER_LOANS_MASTER_HK A1 INNER JOIN OVER_LOANS_CASHFLOW_HK C1

...
fn main() {
    ...
    let app_name = "cf_corporate_guarantee-1.0.0";
    ...
    let total_duration = end_generate_timer
        ...
        .expect("Could not calculate total duration for Corporate Guarantee.");
    info!(
        log,
        "Total duration for Corporate Guarantee cashflow generation: `{:?}`.", total_duration
    );
    println!(
        "Total Duration for Corporate Guarantee cashflow generation: {:2?}",
        total_duration
    );
}

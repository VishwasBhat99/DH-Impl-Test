...
pub fn derive(config_params: ConfigurationParameters, log: &Logger, diag_log: &Logger) {
    ...
    loop {
        ...
        if let Some(dt) = input_account.maturity_dt {
        ...
        if config_params.is_nd() {
            prin_amt = input_account.fwdmtm_in_inr.abs();
        } else {
            prin_amt = input_account.ost_notional_in_inr.abs();
        }
        ...
        let acc_with_cf = log_measurements!(
            diag_log,
            [format!(
        ...
        tot_prin_in_op += acc_with_cf.tot_prin_amt;

        log_measurements!(
            diag_log,
            [format!(
                "Type: WriteAccWithCFs, Identifier: {}",
                acc_with_cf.trade_id
            )],
            writer.write(acc_with_cf)
        );
    }
    writer.close();

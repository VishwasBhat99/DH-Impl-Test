...
impl CP {
    pub fn log_parameters(&self, logger: &Logger) -> () {
        ...
        info!(logger, "avg_bal_sheet_name: {}", self.amb_sheet());
    }
}
...
impl CP {
    fn new_from_matches(matches: clap::ArgMatches) -> CP {
        ...
        let req_fields_file_path = matches.value_of("req_fields_file").unwrap().to_string();
        ...
        let amb_sheet = matches.value_of("amb_sheet").unwrap().to_string();
...
impl CP {
    ...
    pub fn amb_sheet(&self) -> &str {
        return &self.amb_sheet;
    }
}
...
fn get_args_for_app(app_name: &str) -> clap::ArgMatches {
    App::new(app_name)
        ...
        .arg(
            Arg::with_name("amb_sheet")
                .long("amb-sheet-name")
                .value_name("Sheet Name")
                .help("Sheet name of average balance file")
                .required(true)
        )
        .arg(

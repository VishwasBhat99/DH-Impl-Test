...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for the DLOD Pre-Processor.");
    info!(
        log,
        "Total DLOD Pre-Processor Duration: {:?}", total_duration
    );
    println!("Total DLOD Pre-Processor Duration: {:?}", total_duration);
}

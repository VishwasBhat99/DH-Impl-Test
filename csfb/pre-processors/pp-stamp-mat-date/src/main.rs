...
fn main() {
    ...
    let app_name = "pp-stamp-mat-date_csfb";
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for the Stamp Mat Date Pre-Processor.");
    info!(
        log,
        "Total Stamp Maturity Date Pre-Processor Duration: {:?}", total_duration
    );
    println!(
        "Total Stamp Maturity Date Pre-Processor Duration: {:?}",
        total_duration
    );
}

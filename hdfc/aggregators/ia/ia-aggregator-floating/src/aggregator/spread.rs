...
use rbdate::{date_from_timestamp, decr_dt_by_mon_presrv_eom};
...
pub fn get_spread(
    ...
    let rate_vec = bm_rt_map.get(bm_id).expect("Cannot read bm_id");
    let nxt_rep_dt = NaiveDateTime::from_timestamp(next_repr_dt, 0);
    let last_rep_dt = decr_dt_by_mon_presrv_eom(nxt_rep_dt.date(), rep_freq as usize)
        .expect("cannot read last reprising date");
    let mut vec_count = 0;
    let mut vec_date;

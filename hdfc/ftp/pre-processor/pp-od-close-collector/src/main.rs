...
fn main() {
    ...
    let app_name = "pp-od-close-collector-1.0.0";
    ...
    info!(
        log,
        "Total Duration taken by OD pre-processor close collector: {:?}", total_duration
    );
    println!(
        "Total Duration taken by OD pre-processor close collector: {:?}",
        total_duration
    );
}

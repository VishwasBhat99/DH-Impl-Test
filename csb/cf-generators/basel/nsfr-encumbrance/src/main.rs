...
fn main() {
    ...
    let app_name = "cf-nsfr-encumbrance";
    ...
    let total_duration = end_generate_timer
        ...
        .expect("Could not calculate total duration for NSFR Encumbrance.");
    info!(
        log,
        "Total duration for NSFR Encumbrance cashflow generation: `{:?}`.", total_duration
    );
    println!(
        "Total Duration for NSFR Encumbrance cashflow generation: {:2?}",
        total_duration
    );
}

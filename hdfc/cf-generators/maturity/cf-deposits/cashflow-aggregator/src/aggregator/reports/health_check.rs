...
pub(super) struct HealthCheckReport {
    ...
    llgs_with_summary_mismatches: HashMap<
        LLGKey,
...
impl Serialize for HealthCheckReport {
            ...
            S: Serializer,
    {
        ...
        s.serialize_field(
            ...
            &llg_map_to_string_map(
                &self.llgs_with_summary_mismatches
            )
        )?;
        s.serialize_field("groupsWithIncorrectOutputRecords", &self.groups_with_incorrect_output_records)?;
        s.end()
    }
}
...
impl HealthCheckReport {
    ...
    pub(super) fn new_from_report(r: &AggregationReport) -> Self {
        ...
        for (llg, report) in &r.llgs_report.report {
            ...
            accounts_count_across_llgs += report
                .accounts_total_report
                .accounts_count;
            cashflows_count_across_llgs += report
                .accounts_total_report
                .cashflows_count;
            total_principal_amount_across_llgs += report
                .accounts_total_report
                .total_principal_amount;
            total_interest_amount_across_llgs += report
                .accounts_total_report
            ...
            if principal_amount_across_groups.trunc()
                != report.accounts_total_report.total_principal_amount.trunc() {
                llgs_with_mismatched_principal_amount_group_distribution.push(
                    llg.clone()
                )
            }
            if interest_amount_across_groups.trunc()
                != report.accounts_total_report.total_interest_amount.trunc() {
                llgs_with_mismatched_interest_amount_group_distribution.push(
                    llg.clone()
                )
            }
            ...
            if principal_amount_in_summary.round() != report.accounts_total_report.total_principal_amount.round()
                ||
                interest_amount_in_summary.round() != report.accounts_total_report.total_interest_amount.round() {
                ...
                amount_differences_from_llg_to_summary.interest_amount =
                    report.accounts_total_report.total_interest_amount
                ...
                llgs_with_summary_mismatches.insert(
                    llg.clone(),
                    amount_differences_from_llg_to_summary
                );

            }
        }

        let input_to_llgs_account_count_difference = accounts_count_across_llgs
            - r.input_report.accounts_count;
        let input_to_llgs_cashflow_count_difference = cashflows_count_across_llgs
            - r.input_report.cashflows_count;
        let input_to_llgs_principal_amount_difference = total_principal_amount_across_llgs.round()
            - r.input_report.total_principal_amount.round();
        let input_to_llgs_interest_amount_difference = total_interest_amount_across_llgs.round()
            - r.input_report.total_interest_amount.round();
        ...
        for (i, output_count) in r.output_records_report.report.iter().enumerate() {
            if *output_count as usize != expected_output_records_count {
                groups_with_incorrect_output_records.push(
                    format!("group-{}", i)
                )
            }
        }
        ...
        HealthCheckReport {
            ...
            llgs_with_mismatched_principal_amount_file_distribution: llgs_with_mismatched_principal_amount_group_distribution,
            llgs_with_mismatched_interest_amount_file_distribution: llgs_with_mismatched_interest_amount_group_distribution,

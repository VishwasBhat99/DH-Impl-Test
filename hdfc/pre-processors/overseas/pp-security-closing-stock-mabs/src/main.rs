...
fn main() {
    ...
    let app_name = "security_close_stock_mabs";
    ...
    info!(
        log,
        "Total Duration taken by Security Closing Stock MABS pre-processor: {:?}", total_duration
    );
    println!(
        "Total Duration taken by Security Closing Stock MABS pre-processor: {:?}",
        total_duration
    );
}

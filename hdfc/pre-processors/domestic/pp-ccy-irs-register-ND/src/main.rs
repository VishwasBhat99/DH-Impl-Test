...
fn main() {
    ...
    let app_name = "pp_ccy_irs_reg_nd";
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for Bills Pre-processor.");
    info!(
        log,
        "Total Duration for Bills Pre-processor: {:?}", total_duration
    );
    println!(
        "Total Duration for Bills Pre-processor: {:.2?}",
        total_duration
    );
}

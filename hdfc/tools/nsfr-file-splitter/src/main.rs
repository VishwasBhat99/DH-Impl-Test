...
fn main() {
    ...
    let app_name = "pp-basel-file-splitter";
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for Basel File Splitter Pre-processor.");
    info!(
        log,
        "Total Duration for Basel File Splitter Pre-processor: {:?}", total_duration
    );
    println!(
        "Total Duration for Basel File Splitter Pre-processor: {:.2?}",
        total_duration
    );
}

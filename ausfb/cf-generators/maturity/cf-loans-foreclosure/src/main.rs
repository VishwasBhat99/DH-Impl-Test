...
fn main() {
    ...
    let app_name = "cf-loans-foreclosure";
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for Loans Foreclosure.");
    info!(
        log,
        "Total Duration for Loans Foreclosure: {:?}", total_duration
    );
    println!(
        "Total Duration for Loans Foreclosure .cf file generator: {:2?}",
        total_duration
    );
}

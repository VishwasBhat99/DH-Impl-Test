...
pub fn calc_ftp(
            ...
            .iter_mut()
        {
            ...
            if is_cf_req {
                let str_cf = format!(
                    ...
                    residual_days,
                    half_away_from_zero(cf.principal_amount, bal_precision),
                    half_away_from_zero(yield_rate, rate_precision),
                    half_away_from_zero(adj_rate, rate_precision),
                    half_away_from_zero(org_bal_tenor, bal_precision),
                    half_away_from_zero(base_rate_prod, rate_precision),
                    half_away_from_zero(end_rate_prod, rate_precision),
                );
    ...
    total_base_rate_prod = half_away_from_zero(total_base_rate_prod, rate_precision);
    ...
    for i in 0..fix_lst_adjustments.len() {
        ...
        let adj_rate = half_away_from_zero(
            ...
            fix_lst_adjustments[i], st_dt, cf_data_out.account_number
        );
                    DEFAULT_FLOAT
                }
            },
    ...
    if is_lock {
        ...
        let ftp_rates_out = format!(
            ...
            base_rate,
            adj_lock_str,
            half_away_from_zero(cf_data_out.int_rate, rate_precision),
            half_away_from_zero(fix_spread, rate_precision)
        );

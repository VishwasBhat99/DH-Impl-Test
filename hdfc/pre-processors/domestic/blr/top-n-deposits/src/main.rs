...
fn main() {
    ...
    info!(
        diagnostics_log,
        "Start of processing for top n deposits data function."
    );
    processing::process(&config_param, &log, &diagnostics_log);
    info!(
        diagnostics_log,
        "End of processing for top n deposits data function"
    );
    ...
    info!(
        log,
        "Total duration taken for top n deposits: {:?}", total_duration
    );
    println!(
        "Total duration taken for top n deposits: {:?}",
        total_duration
    );
}

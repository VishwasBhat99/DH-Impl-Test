...
pub struct ConfigurationParameters {
    ...
    input_file_lavs: String,
    input_file_lrs: String,
    input_file_lam: String,
    input_file_balm_lrp_flt: String,
...
impl ConfigurationParameters {
    pub fn log_parameters(&self, logger: &Logger) {
        ...
        info!(logger, "input_file_lavs: {}", self.input_file_lavs());
        info!(logger, "input_file_lam: {}", self.input_file_lam());
        info!(logger, "input_file_lrs: {}", self.input_file_lrs());
        info!(
            logger,
            "input_file_balm_lrp_flt: {}",
            self.input_file_balm_lrp_flt()
        );
...
impl ConfigurationParameters {
    fn new_from_matches(matches: clap::ArgMatches) -> ConfigurationParameters {
        ...
        let input_file_icv = matches
            ...
            .to_string();
        let input_file_lavs = matches
            .value_of("input_file_lavs")
            .expect("Error getting `input_file_lavs`.")
            .to_string();
        let input_file_lrs = matches
            .value_of("input_file_lrs")
            .expect("Error getting `input_file_lrs`.")
            .to_string();
        let input_file_balm_lrp_flt = matches
            .value_of("input_file_balm_lrp_flt")
            .expect("Error getting `input_file_balm_lrp_flt`.")
            .to_string();
...
impl ConfigurationParameters {
    ...
    pub fn input_file_lavs(&self) -> &str {
        &self.input_file_lavs
    }
    pub fn input_file_lrs(&self) -> &str {
        &self.input_file_lrs
    }
    pub fn input_file_balm_lrp_flt(&self) -> &str {
        &self.input_file_balm_lrp_flt
    }
...
fn get_eligible_arguments_for_app(app_name: &str) -> clap::ArgMatches {
    App::new(app_name)
        .about("Program For Pre Processing LOANS.")
        .arg(
        ...
        .arg(
            Arg::with_name("input_file_lavs")
                .long("input-file-lavs")
                .value_name("Input File lavs")
                .help("lavs Input file path.")
                .required(true)
        )
        .arg(
        ...
        .arg(
            Arg::with_name("input_file_balm_lrp_flt")
                .long("input-file-balm-lrp-flt")
                .value_name("Input File Balm Lrp Flt")
                .help("Input File Balm Lrp Flt.")
                .required(true)
        )
        .arg(

...
impl RuleBuilder {
    pub(crate) fn build_with(&mut self, rs: IntermediateRuleStep) {
        if self.rule_id != rs.rule_id
            || self.rule_sequence != rs.rule_sequence {
    ...
    pub(crate) fn build(mut self) -> Rule {
        self.all_steps.sort_by(|curr, next| curr.rule_step.partial_cmp(&next.rule_step).expect("Couldn't compare?"));
        ...
        for step in self.all_steps {
            ...
            rule_step_lists[rule_step_list_index].add_rule_step(
                RuleBuilder::make_rule_step(step)
            );
        ...
        if llg_opt.is_none() {
            panic!("Rule number `{}` does not yield an LLG. This is not allowed.", self.rule_id);
        }
...
impl Rule {
    ...
    pub fn does_acc_abide(&self, account: &AccountWithCFs) -> Option<RuleYieldDescriptor> {
        for rule_list in &self.ord_steps {
            match rule_list.does_acc_abide(account) {
                RSListAbidanceRes::True => {
                    let rule_yield_dscr = RuleYieldDescriptor {
                        rule_id: self.id,
                        llg: self.llg
                    };
                    return Some(rule_yield_dscr);
                }
                RSListAbidanceRes::False => {
                    continue;
                }
            }
        }

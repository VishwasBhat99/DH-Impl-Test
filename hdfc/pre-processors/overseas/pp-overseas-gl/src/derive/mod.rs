...
pub fn process(config_param: ConfigurationParameters, log: &Logger, diag_log: &Logger) {
    ...
    if let Some(Ok(reader)) = ref_excel1.worksheet_range(config_param.alm_master_sheet_name()) {
        for row in reader.rows() {
            ref_map.insert(row[1].to_string(), row[9].to_string());
            ia_map.insert(row[1].to_string(), row[7].to_string());
            nsfr_map.insert(row[1].to_string(), row[15].to_string());
    ...
    for line in BufReader::new(input_file).lines().skip(1) {
        ...
        spl_gl_map
            .entry(fields[0].to_string().parse().unwrap_or(DEFAULT_INT))
    ...
    if let Some(Ok(reader)) = gl_moc.worksheet_range(config_param.gl_moc_sheet_name()) {
        for row in reader.rows().skip(1) {
            ...
            ttl_amt.add(dr_bal, cr_bal, net_bal);
            spl_gl_map
                .entry(row[0].to_string().parse().unwrap_or(DEFAULT_INT))
                .and_modify(|val| val.add(dr_bal, cr_bal, net_bal))
                .or_insert(ttl_amt);
    ...
    let header = "seg_1|account_no|seg_5|seg_6|seg_3|seg_8|seg_4|dr_bal|cr_bal|bal_total|concat|alm_line|int_rate|cf_type|gl_prefix|is_acct_gl|ia_line|unknown_fields|cached_size\n";
    output_acc_info.push_str(&header);
    ...
    for lines in BufReader::new(input_file).lines().skip(1) {
        let record = match lines {
            Ok(ln) => ln,
            Err(error) => {
                panic!("error while reading the file : {:?}", error);
            }
        ...
        log_debug!(
            diag_log,
            "NSFR Mapping for Account no: {} is {}",
            fields[1],
            nsfr_mapping
        );

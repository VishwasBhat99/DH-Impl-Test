...
fn main() {
    ...
    let app_name = "bullion-loans-pp-1.1.1";
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for Bullion Loans Preprocessor.");
    info!(
        log,
        "Total Duration for Bullion Loans: {:?}", total_duration
    );
    println!(
        "Total Duration for Bullion Loans Preprocessor: {:?}",
        total_duration
    );
}

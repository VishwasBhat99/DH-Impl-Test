...
fn main() {
    ...
    println!("Total time to process Bills: {:?}", duration);
    let log_str = format!("Total time to process Bills: {:?}", duration);
...
fn initialize() -> FtpParameters {
    let app_name = "FTP-Stamper-UBS-Bills";
...
fn process_records(ftp_parameters: &mut FtpParameters) -> (String) {
    ...
    for mut account_input in ftp_parameters.input_data.iter() {
        ...
        //Read cashflow
        let (cf_input, acc_num) = read_cashflow(
            &account_input,
    ...
    match out_writer.write_all(output.as_bytes()) {
        Ok(_) => println!("Successfully processed all bills accounts ."),
        Err(error) => panic!(

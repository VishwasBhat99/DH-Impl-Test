...
fn main() {
    ...
    let app_name = "repo_lending_and_borrowing";
    ...
    info!(
        log,
        "Total Duration taken by Repo borrowing and lending pre-processor: {:?}", total_duration
    );
    println!(
        "Total Duration taken by Repo borrowing and lending pre-processor: {:?}",
        total_duration
    );
}

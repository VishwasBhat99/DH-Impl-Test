...
fn main() {
    ...
    let app_name = "pp_annex_converter";
    ...
    pre_process::process(&cnfg_params, &log, &diagnostics_log);
    let end_generate_timer = SystemTime::now();
    let total_duration = end_generate_timer
        .duration_since(start_generate_timer)
        .expect("Could not calculate total duration for process.");
    info!(log, "Total duration for process: `{:?}`.", total_duration);
    println!("Total Duration for process: {:2?}", total_duration);
}

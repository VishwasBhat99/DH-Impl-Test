...
fn main() {
    ...
    let app_name = "overseas-exchange-rate";
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for Overseas Exchange Rate Pre-processor.");
    info!(
        log,
        "Total Duration for Overseas Exchange Rate Pre-processor: {:?}", total_duration
    );
    println!(
        "Total Duration for Overseas Exchange Rate Pre-processor: {:.2?}",
        total_duration
    );
}

...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for Finware CASA OD.");
    info!(
        log,
        "Total Duration for Finware CASA OD: {:?}", total_duration
    );
    println!(
        "Total Duration for Finware CASA OD .cf file generator: {:?}",
        total_duration
    );
}

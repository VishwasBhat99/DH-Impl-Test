...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for cashflow aggregation.");
    info!(
        log,
        "Total Duration for cashflow aggregation: {:?}", total_duration
    );
    println!(
        "Total Duration for cashflow aggregation.{:2?}",
        total_duration
    );
}

...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for FRA Pre-processor.");
    info!(
        log,
        "Total Duration for FRA Pre-processor: {:?}", total_duration
    );
    println!(
        "Total Duration for FRA Pre-processor: {:.2?}",
        total_duration
    );
}

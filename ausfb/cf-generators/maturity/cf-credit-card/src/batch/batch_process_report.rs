...
impl BatchProcessReport {
    ...
    pub fn serialise_to_file(&self, file_path: &str) {
        ...
        let mut file_writer =
            io_helpers::buf_file_writer(file_path, "-health-check-report.json", None);
        file_writer
...
impl Serialize for BatchProcessReport {
        ...
        S: Serializer,
    {
        ...
        s.serialize_field("tot_accounts", &self.inputs_processed)?;
        s.serialize_field("acc_read_succ", &self.total_successful_records)?;
        s.serialize_field("acc_read_fail", &self.total_faulty_records)?;
        s.serialize_field("tot_amt_ip", &self.total_amount_input)?;
        s.serialize_field("tot_amt_op", &self.total_principal_output)?;
        s.serialize_field("tot_no_cf", &self.total_cashflows)?;

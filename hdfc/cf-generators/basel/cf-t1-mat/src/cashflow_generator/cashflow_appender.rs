    ...
    match account.remove_cfs_for_key(&keys.cashflows) {
        Ok(cfs) => {
            for cf in cfs {
                ...
                let mut amt = 0.0;
                if config_params.amt_type() == "PRIN" {
                    amt = cf.get_principal_amount();
                } else {
                    amt = cf.get_interest_amount();
                }
                ...
                for val in prd_slabs {
                    if cf_res_days >= val.from && cf_res_days < val.to {
                        ...
                        amt_buckets[bkt_id] += amt;
                    }
                }
            }
        }
        Err(_) => return Err("Could not read cashflows".to_string()),
    }

...
fn main() {
    ...
    let app_name = "cf-non-gsec-investment";
    ...
    let total_duration = end_generate_timer
        ...
        .expect("Could not calculate total duration for Non Gsec Investments.");
    info!(
        log,
        "Total duration for Non Gsec Investments cashflow generation: `{:?}`.", total_duration
    );
    println!(
        "Total Duration for Non Gsec Investments generation: {:2?}",
        total_duration
    );
}

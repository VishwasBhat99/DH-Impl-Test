...
fn main() {
    ...
    let app_name = "security_composition";
    ...
    let total_duration = end_time
        ...
        .expect("Could not calculate total duration for Murex Security Composition Pre-processor.");
    info!(
        log,
        "Total Duration taken by Murex Security Composition Pre-processor: {:?}", total_duration
    );
    println!(
        "Total Duration taken by Murex Security Composition Pre-processor: {:?}",
        total_duration
    );
}

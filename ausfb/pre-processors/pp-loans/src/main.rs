...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for the Pre-Processor loans for AUSFB.");
    info!(
        log,
        "Total Pre-Processor loans of ausfb Duration: {:?}", total_duration
    );
    println!(
        "Total Pre-Processor loans of ausfb Duration: {:?}",
        total_duration
    );
}

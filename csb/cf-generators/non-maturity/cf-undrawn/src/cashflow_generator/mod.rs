...
pub fn generate(config_params: &ConfigurationParameters, log: &Logger, diag_log: &Logger) {
    ...
    loop {
        ...
        if config_params.amt_field_for().to_uppercase() == "NSFR" {
            tot_prin_in_inp += input_account.care_funded;
        } else {
            tot_prin_in_inp += input_account.ccod_undrawn_lcr;
        }
        ...
        let account_with_cashflows = log_measurements!(
            ...
            create_account_with_cashflows(input_account, cashflows, config_params.base_ccy())
        );

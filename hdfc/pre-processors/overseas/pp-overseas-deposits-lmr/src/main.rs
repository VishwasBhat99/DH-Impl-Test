...
fn main() {
    ...
    info!(
        log,
        "Total Duration taken by Overseas Deposits Pre-processor LMR: {:?}", total_duration
    );
    println!(
        "Total Duration taken by Overseas Deposits Pre-processor LMR: {:?}",
        total_duration
    );
}

...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate RD Pre-processor total duration.");
    info!(
        log,
        "Total Duration for RD Pre-processor: {:?}", total_duration
    );
    println!("Total Duration for RD Pre-processor: {:?}", total_duration);
}

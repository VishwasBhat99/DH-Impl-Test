#![cfg_attr(feature = "cargo-clippy", deny(clippy::expect_fun_call))]
#![cfg_attr(
    feature = "cargo-clippy",
    warn(
        clippy::result_unwrap_used,
        clippy::panicking_unwrap,
        clippy::option_unwrap_used
    )
)]
#[macro_use]
extern crate slog;
extern crate chrono;
extern crate clap;
extern crate exitcode;
extern crate rbdate;
extern crate slog_async;
extern crate slog_term;
// #[macro_use]
// extern crate serde_derive;
#[macro_use]
mod macros;
mod configuration_parameters;
mod init;
mod log;
mod process;

use init::init_loggers;
use process::get_exp_day_code;
use std::time::SystemTime;

fn main() {
    let start_timer = SystemTime::now();

    // initialize loggers
    let app_name = "exp-day-code";
    let (config_params, log, diagnostics_log) = init_loggers(app_name);

    // process
    get_exp_day_code(&config_params, &log, &diagnostics_log);

    let total_duration = print_return_time_since!(start_timer);
    log_info!(log, "Total time taken for execution: {:?}", total_duration);
}

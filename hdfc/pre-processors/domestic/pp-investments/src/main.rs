...
fn main() {
    ...
    let total_duration = end_time
        ...
        .expect("Could not calculate total duration for Investment Pre-processor.");
    info!(
        log,
        "Total Duration taken by Investment Pre-processor: {:?}", total_duration
    );
    println!(
        "Total Duration taken by Investment Pre-processor: {:?}",
        total_duration
    );
}

    ...
    loop {
        ...
        if cashflows_for_non_hdfc_account_result.is_err() {
            log_error!(
                ...
                input_account.acc_no,
                cashflows_for_non_hdfc_account_result.err().expect("Unable to unwrap error.");
            );
            continue;
        }
        let mut cashflows = cashflows_for_account_result.expect("Unable to generate cashflows.");
        let mut cashflows_non_hdfc = cashflows_for_non_hdfc_account_result

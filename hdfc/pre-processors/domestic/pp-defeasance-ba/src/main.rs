...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration by Defeasance Pre-Processor.");
    info!(
        log,
        "Total Time Taken By Defeasance Pre-Processor: {:?}", total_duration
    );
    println!(
        "Total Time Taken By Defeasance Pre-Processor: {:?}",
        total_duration
    );
}

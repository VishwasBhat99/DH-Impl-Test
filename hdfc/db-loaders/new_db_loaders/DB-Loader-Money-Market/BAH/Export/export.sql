...
SPOOL &1/BAH/&2/money-market.txt
select A1.Deal_Id
||'~'||A1.Branch
||'~'||A1.Instrument_Name
...
||'~'||CASE WHEN (A1.Lending_Borrowing_Type ='BORROW' AND C1.Interest_Amount<0) THEN
                ABS(C1.Interest_Amount)
                WHEN (A1.Lending_Borrowing_Type ='LEND' AND C1.Interest_Amount>=0) THEN
                C1.Interest_Amount
                ELSE 0
                END
||'~'||C1.Flow_Type

...
pub fn process(config_param: ConfigurationParameters, log: &Logger, diag_log: &Logger) {
    ...
    if let Some(Ok(reader)) = ref_excel3.worksheet_range("Sheet1") {
        for row in reader.rows() {
            ...
            ref_map3.insert(row[0].to_string(), alm_concat);
        }
    }
    ...
    let header = "acc_no|cntr_party|ccy|gl_no|amt|int_rt|st_dt|mat_dt|alm_line|div|cust_id|prod_code|cod_mis_comp_1|dat_val|alm_concat\n";

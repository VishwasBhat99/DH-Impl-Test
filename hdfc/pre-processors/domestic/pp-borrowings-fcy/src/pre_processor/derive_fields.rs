...
fn get_line(
    ...
    let deal_dt = match NaiveDate::parse_from_str(val[5], "%d-%b-%y") {
        Ok(dt) => dt.format("%d-%m-%Y").to_string(),
        Err(_) => "".to_string(),
    };

    let val_dt = match NaiveDate::parse_from_str(val[6], "%d-%b-%y") {
        Ok(dt) => dt.format("%d-%m-%Y").to_string(),
        Err(_) => "".to_string(),
    };
    ...
    output_line.push_str("|F|HDFCCC|");
    output_line.push_str(&gl);
    output_line.push('|');
    output_line.push_str(val[11]);
    output_line.push('|');
    output_line.push_str(&deal_dt.to_string());
    output_line.push('|');
    output_line.push_str(&val_dt.to_string());
    output_line.push('|');
    output_line.push_str(&mat_dt.to_string());
    output_line.push('|');

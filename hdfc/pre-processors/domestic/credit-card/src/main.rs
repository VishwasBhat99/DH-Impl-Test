...
fn main() {
    ...
    pre_processor::process(config_param, &log);
    let end_main_timer = SystemTime::now();
    let total_duration = end_main_timer
        .duration_since(start_main_timer)
        .expect("Could not calculate total duration for Credit card Pre-processor.");
    info!(
        log,
        "Total Duration for Credit card Pre-processor: {:?}", total_duration
    );
    println!(
        "Total Duration for Credit card Pre-processor: {:.2?}",
        total_duration
    );
}

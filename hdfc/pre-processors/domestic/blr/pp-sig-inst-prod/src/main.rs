...
fn main() {
    ...
    info!(
        log,
        "Total Duration taken by Significant Instruments and Products: {:?}", total_duration
    );
    println!(
        "Total Duration taken by Significant Instruments and Products: {:?}",
        total_duration
    );
}

...
use cashflow_gen::account_writer::account_indices::Index;
use macros;
    ...
    pub fn write(&mut self, record: AccountWithCashflows) {
        ...
        if write_result.is_err() {
            panic!(
                ...
                record.tranche_desc,
                write_result.expect_err("Unexpected error occured while wriring cf file.")
            );
        }

        // Write the index for this account:
        self.account_index.set_key(record.tranche_desc.to_string());
...
pub fn flush_contents(mut writer: BufWriter<File>, writer_name: &str) {
    ...
    if flush_result.is_err() {
        panic!(
            ...
            writer_name,
            flush_result.expect_err("Unexpected error occured while flushing buffer writer")
        );
    }
}

...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for the TD Month End Pre-Processor.");
    info!(
        log,
        "Total Time taken by TD Month End Pre-Processor: {:?}", total_duration
    );
    println!(
        "Total Time taken by TD Month End Pre-Processor: {:?}",
        total_duration
    );
}

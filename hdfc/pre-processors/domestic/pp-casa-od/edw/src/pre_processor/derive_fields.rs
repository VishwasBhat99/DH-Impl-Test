...
pub fn get_op_line(
    ...
    let book_bal = rec[4].parse().unwrap_or(DEFAULT_FLOAT);
    let gl_acc_no = if book_bal >= 0.0 { &rec[15] } else { &rec[16] };

    let alm_concat = get_alm_concat(
        &t_ora_mis1,
        t_ora_prod
            .entry(rec[2].to_string())
            .or_insert_with(|| "".to_string()),
        t_ora_gl
...
fn get_line(
    ...
    output_line.push('|');
    output_line.push_str(&remove_comma(&rec[4]));
    ...
    if let Ok(dt) = acc_open_dt {
        ...
        log_error!(
            log,
            "`account_open_date` is not well-formatted for account: `{}`, ERROR: {}.",
            rec[1],
            rec[10]
        );
    ...
    //missing fields
    let cr_int_var = DEFAULT_FLOAT;
    let min_int_var = DEFAULT_FLOAT;
    let max_int_var = DEFAULT_FLOAT;
    let idx_rt = DEFAULT_FLOAT;
    ...
    let gl = if book_bal >= 0.0 { &rec[15] } else { &rec[16] };

...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for PP Loans Preprocessor.");
    info!(
        log,
        "Total Duration for PP Accrued Interest Derivables: {:?}", total_duration
    );
    println!(
        "Total Duration for PP Accrued Interest Derivables Preprocessor: {:?}",
        total_duration
    );
}

...
pub fn process(config_params: &ConfigurationParameters, logger: &Logger, _diag_logger: &Logger) {
    ...
    let rdr = match sdb_io::new_buf_rdr(config_params.extra_file_path()) {
        Ok(r) => r,
        Err(e) => panic!(format!(
            "Cannot read file at path: '{}', Error: '{}'",
            config_params.extra_file_path(),
            e
        )),
    };
    let mut isin_map: HashMap<String, ExtraData> = HashMap::new();
    for (line_num, lines) in rdr.lines().enumerate() {
        ...
        let isin_data = ExtraData {
            yield_val: line_info[1].to_string().parse().unwrap_or(0.0),
            mdur: line_info[2].to_string().parse().unwrap_or(0.0),
        };
        isin_map.insert(line_info[0].to_string(), isin_data);
    }
    ...
    let hft_charge_rules_file_path = format!("{}-hft-charge.txt", config_params.rules_file_path());
    ...
    let hft_charge_rules = AggRules::new_from_path(&hft_charge_rules_file_path, &file_rdr);
    ...
    for account in file_rdr.iter() {
        ...
        let hft_charge_prcnt = get_hft_charge_prcnt(&account, &hft_charge_rules, &config_params);
        ...
        let isin = match account.get_string_for_key(&acc_keys.isin) {
            Ok(val) => val.to_string(),
            Err(_) => format!(""),
        };

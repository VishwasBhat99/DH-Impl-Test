use cashflow_derivator::currency::currency_converter::CurrencyConverter;
use sdb_io;
...
pub fn create_currency_converter(
    ...
    let rdr = match sdb_io::new_buf_rdr(currency_conversion_file_path) {
        ...
        Err(err) => panic!(format!(
            ...
            currency_conversion_file_path, err
        )),
    };
    ...
    for line in rdr.lines() {
        ...
        for component in line
            .expect("Error while reading `Exchange Rate File`.")
            .split('|')
        {
        ...
        if line_components[1] == consolidated_currency {
            ...
            exchange_rate.insert(
                ...
                line_components[2]
                    .parse::<f64>()
                    .expect("Exchange rate is not well-formatted."),
            );
        }
    }

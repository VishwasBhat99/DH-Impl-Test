...
pub fn generate(config_params: &ConfigurationParameters, log: &Logger, diag_log: &Logger) {
    ...
    let mut cfs_prin: Vec<Cashflow> = Vec::new();
    let mut cfs_int: Vec<Cashflow> = Vec::new();
    ...
    loop {
        ...
        if account_opt.is_none() {
            if sum_prin_acc != a_w_cf.outstanding_bal {
                ...
                generate_adjustment_cashflow(&a_w_cf, &mut sum_prin_acc, &mut cfs_prin, log);
            }
            cfs_prin.append(&mut cfs_int);
            a_w_cf.cashflows = protobuf::RepeatedField::from_vec(cfs_prin.clone());
            tot_cfs += cfs_prin.len();
        ...
        tot_int_in_ip += cashflows.int_amt;
        tot_prin_in_ip += cashflows.prin_amt;
        if prev_acc_num == "" {
            tot_acc_encntrd += 1;
            tot_os_in_ip += input_account.outstanding_bal;
            sum_prin_acc += cashflows.prin_amt;
            tot_acc_with_cfs += 1;
        ...
        if input_account.cf_type == "P" {
            cfs_prin.push(cashflows.clone());
            let mut cfs: Vec<Cashflow> = Vec::new();
            cfs.append(&mut cfs_int.clone());
            cfs.append(&mut cfs_prin.clone());
            a_w_cf = append_cashflows(diag_log, &input_account, nxt_rep_dt, &cfs);
        } else if input_account.cf_type == "I" {
            cfs_int.push(cashflows.clone());
            let mut cfs: Vec<Cashflow> = Vec::new();
            cfs.append(&mut cfs_int.clone());
            cfs.append(&mut cfs_prin.clone());
            a_w_cf = append_cashflows(diag_log, &input_account, nxt_rep_dt, &cfs);

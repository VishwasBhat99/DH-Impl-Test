...
fn main() {
    ...
    process::process_data(&config_params, &log);
    let end_main_timer = SystemTime::now();
    let total_duration = end_main_timer
        .duration_since(start_main_timer)
        .expect("Could not calculate total duration for UBS Amb preprocessor.");
    info!(
        log,
        "Total duration for UBS Amb preprocessor: `{:?}`.", total_duration
    );
    println!(
        "Total Duration for UBS AMB preprocessor: {:2?}",
        total_duration
    );
}

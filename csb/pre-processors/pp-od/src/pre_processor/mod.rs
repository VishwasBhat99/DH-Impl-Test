...
use self::structs::cust_master::*;
use self::structs::{alm_master::*, extra_fields::*, input_account::InputAccount, npa::*};
...
pub fn process(config_param: ConfigurationParameters, log: &Logger, diag_log: &Logger) {
    ...
    for (line_num, lines) in input_reader.deserialize().enumerate().skip(1) {
        ...
        let recon_key = ReconKey::new(input_account.balccy, "OD".to_string(), input_account.gl_cd);
        recon
            .store
            .entry(recon_key)
    ...
    output_writer(
        &mut recon_writer,
        recon.print(*config_param.as_on_date(), "OD"),

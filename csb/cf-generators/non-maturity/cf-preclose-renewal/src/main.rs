...
fn main() {
    ...
    let app_name = "preclose-renewal-cf";
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for CASA.");
    info!(log, "Total Duration for CASA: {:?}", total_duration);
    println!(
        "Total Duration for CASA .cf file generator: {:2?}",
        total_duration
    );
}

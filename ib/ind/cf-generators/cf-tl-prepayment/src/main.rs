...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for cf tl prepayment.");
    info!(
        log,
        "Total duration for tl prepayment cashflow generation: `{:?}`.", total_duration
    );
    println!(
        "Total Duration for tl prepayment Cashflow generation: {:2?}",
        total_duration
    );
}

...
pub fn process(config_params: &ConfigurationParameters, log: &Logger, _diag_log: &Logger) {
    ...
    let mut cust_id_vec: HashSet<String> = HashSet::new();
    for line in reader.lines() {
        ...
        cust_id_vec.insert(fields[2].to_string());
    }
    ...
    for line in exrt_reader.lines() {
        ...
        if fields[1] == &files_config.base_ccy {
            exrt_map.insert(
                fields[0].to_string(),
                fields[2].parse::<f64>().unwrap_or_else(|_| {
                    panic!("could not parse exchange rate value for : {}", fields[2])
                }),
            );
        }
    }
    ...
    for file in files_config.files {
        ...
        for account in file_rdr.iter() {
            ...
            let amt_hcy = if files_config.is_consolidated == "true" {
                ost_prin_amt / exrt
            } else {
                ost_prin_amt * exrt
            };

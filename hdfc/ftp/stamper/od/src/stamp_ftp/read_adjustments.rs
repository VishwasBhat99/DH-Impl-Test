...
pub fn read_adj_rates(file_path: &str) -> HashMap<AdjKey, f64> {
    let mut adj_rates: HashMap<AdjKey, f64> = HashMap::new();

    if Path::new(&file_path).exists() {
        let rdr = match sdb_io::new_buf_rdr(file_path) {
            Ok(r) => r,
            Err(e) => panic!("Cannot read file at path: '{}', Error: '{}'", file_path, e),
        };
        ...
        for line in rdr.lines() {
            ...
            let key = AdjKey::new(st_dt, line_components[1].parse::<i32>().unwrap());
...
impl AdjKey {
    pub fn new(start_date: i64, adj_id: i32) -> AdjKey {
        AdjKey { start_date, adj_id }
    }
}

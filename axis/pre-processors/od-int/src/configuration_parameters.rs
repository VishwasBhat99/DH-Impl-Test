...
impl ConfigurationParameters {
    pub fn log_parameters(&self, logger: &Logger) {
        ...
        info!(logger, "input_file_ivs: {}", self.input_file_ivs());
        info!(logger, "output_file: {}", self.output_file_path());
        info!(logger, "schm_types: {}", self.schm_types);
...
impl ConfigurationParameters {
    fn new_from_matches(matches: clap::ArgMatches) -> ConfigurationParameters {
        ...
        let input_file_icv = matches
            ...
            .to_string();
        let input_file_ivs = matches
            .value_of("input_file_ivs")
            .expect("Error getting `input_file_ivs`.")
            .to_string();
        let output_file_path = matches
            .value_of("output_file")
            .expect("Error getting `output_file_path`.")
            .to_string();
        let schm_types = matches
            .value_of("schm_types")
            .expect("Error getting `schm_types`.")
            .to_string();
...
impl ConfigurationParameters {
    ...
    pub fn input_file_ivs(&self) -> &str {
        &self.input_file_ivs
    }
    pub fn output_file_path(&self) -> &str {
        &self.output_file_path
    }
    pub fn schm_types(&self) -> &str {
        &self.schm_types
    }
...
fn get_eligible_arguments_for_app(app_name: &str) -> clap::ArgMatches {
    App::new(app_name)
        .about("Program For Pre Processing OD Interest.")
        .arg(
        ...
        .arg(
            Arg::with_name("input_file_ivs")
                .long("input-file-ivs")
                .value_name("Input File IVS")
                .help("IVS Input file path.")
                .required(true)
        )
        .arg(
        ...
        .arg(
            Arg::with_name("schm_types")
                .long("schm-types")
                .value_name("Scheme Type")
                .help("Scheme types to be considered.")
                .required(true)
        )
        .arg(

...
fn main() {
    ...
    let app_name = "finnacle-pp-td-0.1.2951";
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for the TD Pre-Processor.");
    info!(log, "Total TD Pre-Processor Duration: {:?}", total_duration);
    println!("Total TD Pre-Processor Duration: {:?}", total_duration);
}

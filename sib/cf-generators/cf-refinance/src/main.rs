...
fn main() {
    ...
    cashflow_gen::generate(config_params, &log, &diagnostics_log);

    let end_main_timer = SystemTime::now();
    let total_duration = end_main_timer
        .duration_since(start_main_timer)
        .expect("Could not calculate total duration for Refinance.");
    info!(log, "Total Duration for Refinance: {:?}", total_duration);
    println!(
        "Total Duration for CF REFINANCE .cf file generator: {:2?}",
        total_duration
    );
}

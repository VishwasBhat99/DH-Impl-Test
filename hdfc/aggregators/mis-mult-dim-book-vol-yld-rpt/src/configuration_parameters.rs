...
pub struct ConfigurationParameters {
    ...
    psl_master: String,
    psl_sheet: String,
    rate_bucket_master: String,
    rate_bucket_sheet: String,
...
impl ConfigurationParameters {
    pub fn log_parameters(&self, logger: &Logger) -> () {
        ...
        info!(logger, "rate_bucket_master: {}", self.rate_bucket_master());
        info!(logger, "rate_bucket_sheet: {}", self.rate_bucket_sheet());
    }
}
...
impl ConfigurationParameters {
    fn new_from_matches(matches: clap::ArgMatches) -> ConfigurationParameters {
        ...
        let psl_sheet = matches
            ...
            .to_string();
        let rate_bucket_master = matches
            .value_of("rate_bucket_master")
            .expect("Error getting `rate_bucket_master`.")
            .to_string();
        let rate_bucket_sheet = matches
            .value_of("rate_bucket_sheet")
            .expect("Error getting `rate_bucket_sheet`.")
            .to_string();
...
impl ConfigurationParameters {
    ...
    pub fn rate_bucket_master(&self) -> &str {
        &self.rate_bucket_master
    }
    pub fn rate_bucket_sheet(&self) -> &str {
        &self.rate_bucket_sheet
    }
}
...
fn get_eligible_arguments_for_app(app_name: &str) -> clap::ArgMatches {
    App::new(app_name)
        ...
        .version("1.0.3990")
        .about("This program generates output for MIS Multidimentional Book Volume Yield Report!")
        .arg(
        ...
        .arg(
            Arg::with_name("rate_bucket_master")
                .long("rate-bucket-master")
                .value_name("RATE BUCKET Master File")
                .help("Path to RATE BUCKET Master File")
                .required(true),
        )
        .arg(
            Arg::with_name("rate_bucket_sheet")
                .long("rate-sheet")
                .value_name("RATE BUCKET Master Sheet Name")
                .help("Name of RATE BUCKET Master File sheet")
                .default_value("Sheet1")
                .required(false),
        )
        .arg(

...
fn main() {
    ...
    let app_name = "security_closing_stock_lcr";
    ...
    info!(
        log,
        "Total Duration taken by Security Closing Stock LCR pre-processor: {:?}", total_duration
    );
    println!(
        "Total Duration taken by Security Closing Stock LCR pre-processor: {:?}",
        total_duration
    );
}

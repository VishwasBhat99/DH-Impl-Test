...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for Undrawn.");
    info!(log, "Total Duration for Undrawn: {:?}", total_duration);
    println!(
        "Total Duration for Undrawn .cf file generator: {:2?}",
        total_duration
    );
}

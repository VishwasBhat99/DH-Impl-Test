...
pub fn derive_cashflows(
    ...
    let mat_dt: i64;
    let mut cf_dt: i64 = timestamp(account.due_dt.expect(&format!(
        "Due date not found for reference no. {}",
        account.reference
    )));
    let as_on_dt: i64;
    ...
    if let Some(amt) = account.cf_amt {
        if account.component == "PRINCIPAL" || account.component == "FROZEN" {
            prin_amt = amt;
            if prin_amt <= 0.0 {
                log_error!(
                    log,
        ...
        if cf_dt > mat_dt {
            cf_dt = timestamp(
                npa_cfdate_adjusment(
                    account.mat_dt.expect("Date is not in correct format"),
                    account.npa_typ.to_string(),
                )
            ...
            if let Some(dt) = npa_cfdate_adjusment(
                account.due_dt.expect("Date is not in correct format"),
                account.npa_typ.to_string(),
            ) {
                cf_dt = timestamp(dt);
            } else {
                ...
                cf_dt = if let Some(m_dt) = npa_cfdate_adjusment(
                    account.mat_dt.expect("Date is not in correct format"),
                    account.npa_typ.to_string(),
                ) {
                    timestamp(m_dt)

...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for GL MOC.");
    info!(log, "Total Duration for GL MOC: {:?}", total_duration);
    println!(
        "Total Duration for GL MOC .cf file generator: {:2?}",
        total_duration
    );
}

...
pub fn generate(config_params: &ConfigurationParameters, log: &Logger, diag_log: &Logger) {
    ...
    loop {
        ...
        total_principal_in_input += input_account.principal_ouststanding_amount;
        if input_account.maturity_date == def_dt {
            let cashflows_for_account_result = log_measurements!(
                ...
                generate_cashflows(config_params.as_on_date(), &mut input_account, log,)
            );
            ...
            tot_prin_in_op += account_with_cashflows.principal_ouststanding_amount;
            log_measurements!(
                diag_log,
                [format!(

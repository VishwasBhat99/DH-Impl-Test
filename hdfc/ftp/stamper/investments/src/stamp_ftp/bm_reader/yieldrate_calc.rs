...
pub fn calc_yieldrate(
    ...
    let end_date = rbdate::increment_date_by_months(date, 6 as u16);
    let residual_tenor = rbdate::num_days_start_to_end(date, end_date);
    ...
    for i in 0..lst_bm.len() {
        if !(i == lst_bm.len() - 1) {
            ...
            if days_diff >= lst_bm[i].days_diff && days_diff <= lst_bm[i + 1].days_diff {
                //calculate rate using daily interpolation
                if days_diff <= residual_tenor {

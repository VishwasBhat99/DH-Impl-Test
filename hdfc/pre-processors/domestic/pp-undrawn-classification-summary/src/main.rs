...
fn main() {
    ...
    let app_name = "pp_undrawn_summary";
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration by Undrawn Pre-Processor.");
    info!(
        log,
        "Total Time Taken By Undrawn Pre-Processor: {:?}", total_duration
    );
    println!(
        "Total Time Taken By Undrawn Pre-Processor: {:?}",
        total_duration
    );
}

...
fn main() {
    ...
    let total_duration = end_generate_timer
        ...
        .expect("Could not calculate total duration for Undrawn Individual.");
    info!(
        log,
        "Total duration for Undrawn cashflow generation: `{:?}`.", total_duration
    );
    println!(
        "Total Duration for Undrawn cashflow generation: {:2?}",
        total_duration
    );
}

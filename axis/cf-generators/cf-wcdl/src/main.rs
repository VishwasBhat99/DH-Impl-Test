...
fn main() {
    ...
    let total_duration = end_generate_timer
        ...
        .expect("Could not calculate total duration for WCDL.");
    info!(
        log,
        "Total duration for WCDL Cashflow Generation: `{:?}`.", total_duration
    );
    println!(
        "Total Duration for WCDL Cashflow Generation: {:2?}",
        total_duration
    );
}

...
pub fn generate(config_params: &ConfigurationParameters, log: &Logger, diag_log: &Logger) {
    ...
    loop {
        ...
        if let Some(initial_deposit_amount) = input_account.initial_deposit_amount {
            tot_prin_in_in += initial_deposit_amount;
        }
        let cashflows_for_account_result = log_measurements!(
            diag_log,
            [format!(
                "Type: GenCFs, Identifier: `{}`",
                input_account.bank_code
            )],
        ...
        tot_prin_in_op += account_with_cashflows.initial_deposit_amount;
        tot_int_in_op += account_with_cashflows.interest_accrued;
        log_measurements!(
            diag_log,
            [format!(
                "Type: WriteAccWithCFs, Identifier: {}",
                account_with_cashflows.bank_code
            )],

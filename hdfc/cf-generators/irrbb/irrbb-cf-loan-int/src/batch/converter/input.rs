    ...
    pub fn new_from_line(line: &str, dmy: &DateParser) -> InputParseResult {
        ...
        let input = Input {
            ...
            weaker_desc: match value_iterator.next() {
                ...
                None => {
                    return InputParseResult::Error(format!(
                        "Could not parse property `weaker_desc`."
                    ));
                }
            },
            int_income_gl: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return InputParseResult::Error(format!(
                        "Could not parse property `int_income_gl`."
                    ));
                }
            },
            overdue_int_gl: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return InputParseResult::Error(format!(
                        "Could not parse property `overdue_int_gl`."
                    ));
                }
            },
            int_on_cancellation_gl: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return InputParseResult::Error(format!(
                        "Could not parse property `int_on_cancellation_gl`."
                    ));
                }
            },
            writeoff_gl: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return InputParseResult::Error(format!(
                        "Could not parse property `writeoff_gl`."
                    ));
                }
            },
            int_income_gl_amt: match value_iterator.next() {
                Some(val) => val.parse().unwrap_or(0.0),
                None => {
                    return InputParseResult::Error(format!(
                        "Could not parse property `int_income_gl_amt`."
                    ));
                }
            },
            overdue_int_gl_amt: match value_iterator.next() {
                Some(val) => val.parse().unwrap_or(0.0),
                None => {
                    return InputParseResult::Error(format!(
                        "Could not parse property `overdue_int_gl_amt`."
                    ));
                }
            },
            int_on_cancellation_gl_amt: match value_iterator.next() {
                Some(val) => val.parse().unwrap_or(0.0),
                None => {
                    return InputParseResult::Error(format!(
                        "Could not parse property `int_on_cancellation_gl_amt`."
                    ));
                }
            },
            writeoff_gl_amt: match value_iterator.next() {
                Some(val) => val.parse().unwrap_or(0.0),
                None => {
                    return InputParseResult::Error(format!(
                        "Could not parse property `writeoff_gl_amt`."
                    ));
                }
            },

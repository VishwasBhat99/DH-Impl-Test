...
pub fn process(config_param: ConfigurationParameters, log: &Logger, diag_log: &Logger) {
    ...
    let spread_reader = match new_buf_rdr(config_param.penalty_file()) {
        Ok(file) => file,
        Err(error) => panic!(
            "Could not found spread file: `{}` on location `{}` : {}.",
            config_param.penalty_file(),
            current_dir()
                .expect("Error while getting current directory path.")
                .display(),
            error
    ...
    for (line_no, lines) in spread_reader.lines().enumerate() {
        let spread_line = match lines {
        ...
        let spread_fields = spread_line.split(config_param.penalty_file_separator()).collect::<Vec<&str>>();

        spread_map.insert(
            spread_fields[0].to_string(),
            spread_fields[1].to_string() + "|" + spread_fields[2],
        );
    }
    ...
    for (line_num, lines) in reader.deserialize().enumerate() {
        ...
        if spread_map.get(&input_account.foracid).is_some() {
            append_data = spread_map
                .get(&input_account.foracid)

...
pub fn cashflows_grouped_by_day(
    ...
    let interest_rate = account
        .get_f64_for_key(&k.interest_rate)
        .expect("Cannot read interest rate from .cf file.");
    let mut repricing_date: Option<NaiveDate> = match account.get_i64_for_key(&k.next_rep_date) {
        Ok(val) => {
            if val == 0 {
                None
            } else {
    ...
    for cf in cfs.iter_mut() {
        ...
        if &cf_date <= as_on_date {
            if default_overdue_llg_path == "NA" {

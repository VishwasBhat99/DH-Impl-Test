...
impl RuleStep {
    pub fn does_acc_abide(&self, account: &AccountWithCFs) -> bool {
        match &self.comparator {
            ...
            Comparator::EQ(expected_val) => {
                match expected_val {
                    Value::I32(e_v) => {
                        return e_v == &account.get_i32_for_key(&self.field_name).expect("couldn't compare");
                    },
                    Value::F32(e_v) => {
                        return e_v == &account.get_f32_for_key(&self.field_name).expect("couldn't compare");
                    },
                    Value::I64(e_v) => {
                        return e_v == &account.get_i64_for_key(&self.field_name).expect("couldn't compare");
                    },
                    Value::F64(e_v) => {
                        return e_v == &account.get_f64_for_key(&self.field_name).expect("couldn't compare");
                    },
                    Value::String(e_v) => {
                        return e_v == account.get_string_for_key(&self.field_name).expect("couldn't compare");
                    },
                }
            },
            Comparator::LT(expected_val) => {
                match expected_val {
                    Value::I32(e_v) => {
                        return e_v > &account.get_i32_for_key(&self.field_name).expect("couldn't compare");
                    },
            ...
            Comparator::BTWN(expected_vals) => {
                ...
                match expected_type {
                    Type::I32 => {
                        match (&expected_vals.0, &expected_vals.1) {
                            (Value::I32(v1), Value::I32(v2)) => {
                                let act_val = &account.get_i32_for_key(&self.field_name).expect("couldn't compare");
                    ...
                    Type::F32 => {
                        match (&expected_vals.0, &expected_vals.1) {
                            (Value::F32(v1), Value::F32(v2)) => {
                                let act_val = &account.get_f32_for_key(&self.field_name).expect("couldn't compare");
                                return v1 < act_val && act_val < v2;
                            },
                            _ => {
                                panic!("")
                    ...
                    Type::I64 => {
                        match (&expected_vals.0, &expected_vals.1) {
                            (Value::I64(v1), Value::I64(v2)) => {
                                let act_val = &account.get_i64_for_key(&self.field_name).expect("couldn't compare");
                                return v1 < act_val && act_val < v2;
                            },
                            _ => {
                                panic!("")
                    ...
                    Type::F64 => {
                        match (&expected_vals.0, &expected_vals.1) {
                            (Value::F64(v1), Value::F64(v2)) => {
                                let act_val = &account.get_f64_for_key(&self.field_name).expect("couldn't compare");
                                return v1 < act_val && act_val < v2;
                            },
                            _ => {
                                panic!("")
                    ...
                    Type::String => {
                        match (&expected_vals.0, &expected_vals.1) {
                            (Value::String(v1), Value::String(v2)) => {
                                let act_val = account.get_string_for_key(&self.field_name).expect("couldn't compare");
                                return v1 < act_val && act_val < v2;
                            },
                            _ => {
                                panic!("")

...
pub fn split_by_defesance(
    ...
    apply_defesance: bool,
) {
    let mut field_num = 1;
    let mut maturity_date;
    if apply_defesance {
        //Existing defesance logic.
        maturity_date = NaiveDate::parse_from_str(&mat_date, "%d-%m-%Y")
        ...
        //Use vw_val_date as the cf-date in HMT logic.
        maturity_date = NaiveDate::parse_from_str(fields[21], "%d-%m-%Y").unwrap_or(as_on_date);
    }
    for field in fields {
        if field_num == 48 {

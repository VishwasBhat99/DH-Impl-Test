use cashflow_generator::account_without_cashflows::Account;
use cashflow_generator::account_without_cashflows_writer::account_indices::Index;
use macros;
...
pub struct AccountWithOutCashflowsWriter {
    log: Logger,
...
impl AccountWithOutCashflowsWriter {
    pub fn new(path: &str, log: &Logger) -> AccountWithOutCashflowsWriter {
        AccountWithOutCashflowsWriter {
    ...
    pub fn write(&mut self, record: Account) {
        ...
        let output_bytes = match record.write_length_delimited_to_bytes() {
            ...
            Err(e) => {
                log_error!(
                    self.log,
                    "Couldn't parse to bytes. Aborting execution. On as_on_date: '{}' Error: {:?}.",
                    record.as_on_date,
                    e
                );
                return;
        ...
        self.account_index.set_key(record.as_on_date.to_string());
...
impl AccountWithOutCashflowsWriter {
    pub fn close(self) {
        flush_contents(self.account_w_cf_writer, "AccountWithOutCashflowsWriter")
    }
}
...
pub fn flush_contents(mut writer: BufWriter<File>, writer_name: &str) {
    ...
    if flush_result.is_err() {
        panic!(
            ...
            writer_name,
            flush_result.err().expect("Unable to get flush error!")
        );
    }
}

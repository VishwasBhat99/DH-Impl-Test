...
pub fn generate(config_params: &ConfigurationParameters, log: &Logger, diag_log: &Logger) {
    ...
    loop {
        ...
        tot_acc_encntrd += 1;
        succ_rec += 1;
        tot_prin_in_inp += input_account.cash;

        let account_without_cashflows = log_measurements!(
            diag_log,
            [format!(
                "Type: CreateAccWithCFs, Identifier: {}",
                input_account.member_id
            )],
            create_account_without_cashflows(input_account, config_params.base_ccy())
        );
        tot_prin_in_op += account_without_cashflows.cash;

        log_measurements!(
            diag_log,
            [format!(
                "Type: WriteAccWithCFs, Identifier: {}",
                account_without_cashflows.member_id
            )],

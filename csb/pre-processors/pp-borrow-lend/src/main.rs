...
fn main() {
    ...
    info!(
        log,
        "Total duration taken by Borrowings & Lendings pre-processor: {:?}", total_duration
    );
    println!(
        "Total duration taken by Borrowings & Lendings pre-processor: {:?}",
        total_duration
    );
}

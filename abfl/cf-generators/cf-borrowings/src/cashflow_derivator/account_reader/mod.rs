...
use crate::cashflow_derivator::account_reader::input_account_reader_report::InputAccountReaderReport;
use crate::macros;
use slog::Logger;
use std::fs::File;
...
impl InputAccountReader {
    pub fn new(src_file: &str, log: &Logger) -> InputAccountReader {
        let dmy_date_parser = rbdate::DateParser::new("%d-%m-%Y".to_string(), false);
        let file = match sdb_io::new_buf_rdr(src_file) {
            Ok(file) => file,

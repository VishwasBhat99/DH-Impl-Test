...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for the GL Pre-Processor.");
    info!(
        log,
        "Total Duration for Processing GL-PP Data: {:?}", total_duration
    );
    println!(
        "Total Duration for Processing GL-PP Data: {:?}",
        total_duration
    );
}

...
fn main() {
    ...
    let app_name = "finnone-structured-loans-cf-1.1.1";
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for Finnone Structured Loans.");
    info!(
        log,
        "Total duration for Finnone Structured Loans cashflow generation: `{:?}`.", total_duration
    );
    println!(
        "Total Duration for Finnone Structured Loans cashflow generation: {:2?}",
        total_duration
    );
}

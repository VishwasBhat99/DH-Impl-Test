...
pub fn create_account_with_cashflows(
    ...
    let res_days = if let Some(mat_dt) = acc.maturity_dt {
        if mat_dt > *config_params.as_on_date() {
            rbdate::num_days_start_to_end(*config_params.as_on_date(), mat_dt)
        } else {
            rbdate::num_days_start_to_end(mat_dt, *config_params.as_on_date())
        }
    } else {
        0
    };
    ...
    out_acc.residual_maturity = res_days;

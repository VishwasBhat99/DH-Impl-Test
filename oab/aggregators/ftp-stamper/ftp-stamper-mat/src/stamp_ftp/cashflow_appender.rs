...
pub fn append_cashflow(
    ...
    cfoutput.account_id = match cfin.get_string_for_key(&required_fields.account_number) {
        Ok(result) => result,
        Err(_) => match cfin.get_string_for_key(&required_fields.account_number) {
            Ok(result) => result,
            Err(err) => {
    ...
    cfoutput.gl_code = match cfin.get_i64_for_key(&required_fields.gl_code) {
        Ok(val) => val.to_string(),
        Err(_) => match cfin.get_string_for_key(&required_fields.gl_code) {
            Ok(val) => val.to_string(),

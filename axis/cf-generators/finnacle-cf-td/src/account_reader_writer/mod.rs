...
pub fn generate(config_params: &ConfigurationParameters, log: &Logger, _diag_log: &Logger) {
    ...
    let (mut reader, mut writer) = create_io_workers(
        config_params.input_file_path(),
        config_params.output_file_path(),
        log,
    );
    loop {
        let account_opt = reader.next();
        ...
        let account_with_cashflows =
            create_account_with_cashflows(input_account, config_params.source_name(), cashflows);

...
pub fn get_dim(
    ...
    dim_slabs: &Vec<RangeSlab>,
    srcmap_slabs: &Vec<MapSlab>,
) -> String {
...
pub fn naivedate_from_timestamp(t: i64) -> NaiveDate {
    let naive_date_time = NaiveDateTime::from_timestamp_opt(t, 0).expect(&format!(
        "can not convert NaiveDateTime from timestamp :{}",
        t
    ));

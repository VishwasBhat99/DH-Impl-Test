...
impl CashflowOrganizer {
    ...
    pub fn organize(
        ...
        if !overdue_cashflows.is_empty() {
            for (cf_date, aggregated_data) in overdue_cashflows.iter() {
                let mut overdue_cashflow = HashMap::new();
                overdue_cashflow.insert(config_params.as_on_date().succ(), *aggregated_data);
                let residual_period =
                    num_days_start_to_end(*cf_date, *config_params.as_on_date()) as i32;
                let llg_category = get_overdue_llg(residual_period, &overdue_llg_map);

                let default_overdue_llg =
                    LLGKey::new(llg.currency.clone(), llg_category, llg.cf_type.clone());
    ...
    pub fn build_with(
        ...
        if input_ccy == config_params.src_local_ccy() {
            ...
            base_llg = LLGKey::new(llg.currency.to_string(), llg.category, llg.cf_type.clone());
        }

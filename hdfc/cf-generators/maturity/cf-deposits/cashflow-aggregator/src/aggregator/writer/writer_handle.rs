...
impl WriterHandles {
    ...
    pub fn send_to_writer(&self, writer_pos: usize, record: LLGAggregatedRecord) {
        self.handles[writer_pos]
            ...
            .expect(
                &format!(
                        "Unable to send record to writer. Writer number: {}", writer_pos
                    )
            )
    }
    ...
    pub fn end_writer(self) -> AggregateWriterReport {
        let mut writer_thread_reports= Vec::with_capacity(6);
        for handle in self.handles {
            drop(handle.aggregate_sender);
            let report = handle.report_receiver.recv().expect(
                "Could not recieve report from writer thread."
            );
            writer_thread_reports.insert(
                report.thread_no as usize,
                report
            );

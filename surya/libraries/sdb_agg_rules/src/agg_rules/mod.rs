...
impl AggRules {
    ...
    pub fn llg_for_acc(&self, account: &AccountWithCFs) -> Option<RuleYieldDescriptor> {
        for rule in &self.ord_rules {
            if let Some(rule_yield_descr) = rule.does_acc_abide(account) {
                return Some(rule_yield_descr);
            } else {
                continue;
            }
        }
...
pub fn get_all_llgs(path: &str) -> Result<Vec<i32>, String> {
    let mut llgs: Vec<i32> = Vec::new();
    let llg_path = match new_buf_rdr(path) {
        Ok(file) => file,
        Err(error) => return Err(format!("Cannot read path: {}", error)),
    };
    for (i, line) in llg_path.lines().enumerate() {
        let line = line.expect("Could Not Read Line").to_string();
        let rule_fields: Vec<&str> = line.split('|').collect();
        if !["AND", "OR"].contains(&rule_fields[6]) {
            llgs.push(rule_fields[6].parse::<i32>().unwrap())
        }
    }
    Ok(llgs)
}

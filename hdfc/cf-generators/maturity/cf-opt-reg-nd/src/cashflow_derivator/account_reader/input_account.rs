...
pub struct InputAccount {
    ...
    pub m_barrier_style: String,
    pub m_barrier_type: String,
    pub m_barrier_1: String,
    pub m_barrier_2: String,
    pub m_window_from: String,
    pub m_window_end: String,
    pub digital_payout_currency: String,
...
impl<'a> InputAccount {
    pub fn new_from_line(
        ...
        let input_account = InputAccount {
            ...
            settle_ccy: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `settlement_currency`.");
                }
            },
            fwdmtm_settle_ccy: match value_iterator.next() {
                Some(val) => val.parse().unwrap_or(DEFAULT_FLOAT),
                None => {
                    return Err("Could not parse property `FWDMTM settlement_currency`.");
                }
            },
            settlementdate: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `settlementdate`.");
                }
            },
            settlement_amount: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `settlement_amount`.");
                }
            },
            m_barrier_style: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `m_barrier_style`.");
                }
            },
            m_barrier_type: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `m_barrier_type`.");
                }
            },
            m_barrier_1: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `m_barrier_1`.");
                }
            },
            m_barrier_2: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `m_barrier_2`.");
                }
            },
            m_window_from: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `m_window_from`.");
                }
            },
            m_window_end: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `m_window_end`.");
                }
            },
            digital_payout_currency: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `digital_payout_currency`.");
                }
            },
            settlement_type: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `settlement_type`.");
                }
            },
        };
        Ok(input_account)

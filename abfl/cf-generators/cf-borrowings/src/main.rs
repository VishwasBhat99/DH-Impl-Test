...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for Borrowings.");
    info!(log, "Total Duration for Borrowings: {:?}", total_duration);
    println!(
        "Total Duration for Borrowings cashflow file generator: {:2?}",
        total_duration
    );
}

...
pub fn create_account_with_cashflows(
    ...
    out_acc.pre_emi_ovd_amt = account.pre_emi_ovd_amt;
    out_acc.exc_pre_emi_amt = account.exc_pre_emi_amt;
    out_acc.emi_ovd_amt = account.emi_ovd_amt;
    out_acc.exc_emi_amt = account.exc_emi_amt;
    out_acc.pre_emi_ovd_gl_cd = account.pre_emi_ovd_gl_cd;
    out_acc.exc_pre_emi_gl_cd = account.exc_pre_emi_gl_cd;
    out_acc.emi_ovd_gl_cd = account.emi_ovd_gl_cd;
    out_acc.exc_emi_gl_cd = account.exc_emi_gl_cd;
    out_acc.security = account.security;
    out_acc.ncd_type = account.ncd_type;
    out_acc.holder_type = account.holder_type;
    ...
    //Adjustment Cashflow 
    //Writing cashflow with cf-amt = (remaining amount from outstanding-balance) to maturity-date 
    let mut rem_cf = Cashflow::new();
    let mut all_cfs = protobuf::RepeatedField::from_vec(cashflows);
    rem_cf.prin_amt = out_acc.prin_ost_bal - tot_prin_amt;
    rem_cf.date = out_acc.mat_dt;
    all_cfs.push(rem_cf);

...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for the cc_gl_classifier.");
    info!(log, "Total cc_gl_classifier Duration: {:?}", total_duration);
    println!("Total cc_gl_classifier Duration: {:?}", total_duration);
}

...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for TD.");
    info!(log, "Total Duration for TD: {:?}", total_duration);
    println!(
        "Total Duration for TD .cf file generator: {:2?}",
        total_duration
    );
}

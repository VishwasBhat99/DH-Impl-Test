...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration by Undrawn Classifications pre-processor.");
    info!(
        log,
        "Total Time Taken By Undrawn Classification Pre-Processor: {:?}", total_duration
    );
    println!(
        "Total Time Taken By Undrawn Classification Pre-Processor: {:?}",
        total_duration
    );
}

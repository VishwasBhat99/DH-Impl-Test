...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for Securitisation Investments.");
    info!(
        log,
        "Total Duration for Securitisation Investments Pre-processor: {:?}", total_duration
    );
    println!(
        "Total Duration for Securitisation Investments Pre-processor: {:.2?}",
        total_duration
    );
}

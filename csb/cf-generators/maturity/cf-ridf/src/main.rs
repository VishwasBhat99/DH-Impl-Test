...
fn main() {
    ...
    let total_duration = end_generate_timer
        ...
        .expect("Could not calculate total duration for Ridf.");
    info!(
        log,
        "Total duration for Ridf cashflow generation: `{:?}`.", total_duration
    );
    println!(
        "Total Duration for Ridf cashflow generation: {:2?}",
        total_duration
    );
}

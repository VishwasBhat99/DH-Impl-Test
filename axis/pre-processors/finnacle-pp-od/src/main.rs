...
fn main() {
    ...
    let app_name = "finnacle-pp-od-1.0.2958";
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for the OD Pre-Processor.");
    info!(log, "Total OD Pre-Processor Duration: {:?}", total_duration);
    println!("Total OD Pre-Processor Duration: {:?}", total_duration);
}

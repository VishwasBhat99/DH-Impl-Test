...
impl ConfigurationParameters {
    pub fn log_parameters(&self, logger: &Logger) {
        ...
        info!(logger, "concat: {}", self.concat());
    }
}
...
impl ConfigurationParameters {
    fn new_from_matches(matches: clap::ArgMatches) -> ConfigurationParameters {
        ...
        let concat = matches
            .value_of("concat")
            .expect("Error getting `concat` value.")
            .to_string();
...
fn get_eligible_arguments_for_app(app_name: &str) -> clap::ArgMatches {
    App::new(app_name)
        .about("This app modifies data to conform with the input requirements of Murex Security Closing Stock MABS PP!")
        .version("1.1.2024")

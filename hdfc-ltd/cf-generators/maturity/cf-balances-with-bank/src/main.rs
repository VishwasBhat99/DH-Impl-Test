...
fn main() {
    ...
    let app_name = "cf-balances-wt-bank";
    ...
    let total_duration = end_generate_timer
        ...
        .expect("Could not calculate total duration for Balances With Bank.");
    info!(
        log,
        "Total duration for Balances With Bank cashflow generation: `{:?}`.", total_duration
    );
    println!(
        "Total Duration for Balances With Bank cashflow generation: {:2?}",
        total_duration
    );
}

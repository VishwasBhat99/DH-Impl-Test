...
fn main() {
    ...
    let total_duration = end_generate_timer
        ...
        .expect("Could not calculate total duration for Term Deposits.");
    info!(
        log,
        "Total duration for Term Deposits cashflow generation: `{:?}`.", total_duration
    );
    println!(
        "Total Duration for Term Deposits cashflow generation: {:2?}",
        total_duration
    );
}

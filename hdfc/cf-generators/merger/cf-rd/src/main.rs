...
#[macro_use]
extern crate slog;
extern crate additional_passthroughs;
...
fn main() {
    ...
    let total_duration = end_generate_timer
        ...
        .expect("Could not calculate total duration for Term Deposits Merger.");
    info!(
        log,
        "Total duration for Term Deposits Merger cashflow generation: `{:?}`.", total_duration
    );
    println!(
        "Total Duration for Term Deposits Merger cashflow generation: {:2?}",
        total_duration
    );
}

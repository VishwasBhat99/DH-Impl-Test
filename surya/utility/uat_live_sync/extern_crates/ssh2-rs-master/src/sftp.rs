...
impl Sftp {
    ...
    #[doc(hidden)]
    pub fn exist(&self, file: &Path) -> bool {
        match self.open(file.as_ref()) {
            Ok(_) => true,
            Err(_) => false,
        }
    }

    //this functions creates all directories recursively
    #[doc(hidden)]
    pub fn create_dir_all(&self, path: &Path) -> Result<(), Error> {
        ...
        match self.mkdir(path, 0o777) {
            Ok(()) => return Ok(()),
            Err(ref e) if e.code() as i32 == -31 => {}
            Err(_) if self.exist(path) => return Ok(()),
            Err(_e) => return Err(Error::new(1, "unable to create")),
        }
        match path.parent() {
            Some(p) => self.create_dir_all(p)?,
            None => return Err(Error::new(1, "failed to create whole tree")),
        }
        match self.mkdir(path, 0o777) {
            Ok(()) => Ok(()),
            Err(_) if self.exist(path) => Ok(()),
            Err(e) => Err(e),
        }
    }

    //copies a file from and to remote server
    #[doc(hidden)]
    pub fn copy(
        &self,
        source_path: &str,
        ...
        let mut source = match self.open(source_path.as_ref()) {
            Ok(file) => file,
            Err(err) => return Err(err),
        };

        if let Err(_err) = self.create_dir_all(Path::new(dest_path).parent().unwrap()) {
        }
        let mut target = match self.create(dest_path.as_ref()) {
            Ok(file) => file,
            Err(err) => return Err(err),
        };
        let src_stat = source.stat().unwrap();
        let pb = ProgressBar::new(
            src_stat
                .size
                .expect("unable to get source meta while copying"),
        );
        pb.set_style(ProgressStyle::default_bar()
            .template("{spinner:.green} [{elapsed_precise}] [{bar:40.green/black}] {bytes}/{total_bytes} ({eta})")
            .progress_chars("#>-"));
        let status: Result<u64, std::io::Error>;
        if show_progress {
            status = std::io::copy(&mut pb.wrap_read(source), &mut target);
        } else {
            status = std::io::copy(&mut source, &mut target);
        }
        let mut dest_stat = target.stat().unwrap();

        dest_stat.perm = Some(0o777);

        if let Err(err) = self.setstat(dest_path.as_ref(), dest_stat) {
            println!("{}", err);
        }
        if status.is_ok() {
            return Ok(());
        } else {
            return Err(Error::new(4, "Unable to Copy"));
        }
    }

    //copies a file from local path to remote path(Upload)
    #[doc(hidden)]
    pub fn copy_to_remote(
        ...
        let mut source: std::fs::File = match std::fs::File::open(local_path) {
            Ok(file) => file,
            Err(_err) => return Err(Error::new(3, "unable to read local file")),
        };

        if let Err(_err) = self.create_dir_all(Path::new(remote_path).parent().unwrap()) {
        }
        let mut target = match self.create(remote_path.as_ref()) {
            Ok(file) => file,
            Err(err) => return Err(err),
        };
        ...
        let mut stat = target.stat().unwrap();
        stat.perm = Some(0o777);
        // self.setstat(remote_path.as_ref(), stat);
        if let Err(err) = self.setstat(remote_path.as_ref(), stat) {
            println!("{}", err);
        }
        let _message = format!("Uploaded `{}`", local_path);
        if status.is_ok() {
            return Ok(());
        } else {
            return Err(Error::new(4, "Unable to upload"));
        }
    }

    //copies a file from remote server to local(Download)
    #[doc(hidden)]
    pub fn copy_from_remote(
        ...
        let mut source = match self.open(remote_path.as_ref()) {
            Ok(file) => file,
            Err(err) => return Err(err),
        };
        match std::fs::create_dir_all(Path::new(local_path).parent().unwrap()) {
            _ => {}
        }
        let mut target = match std::fs::File::create(local_path) {
            Ok(file) => file,
            Err(_err) => {
                return Err(Error::new(3, "unable to Create local file"));
            }
        };
        let src_stat = source.stat().unwrap();
        let pb = ProgressBar::new(
        ...
        match std::fs::set_permissions(local_path, std::fs::Permissions::from_mode(0o777)) {
            Ok(_) => {}
            Err(_) => {
                return Err(Error::new(5, "Unable to Set File permission"));
            }
        }
        let _message = format!("Uploaded `{}`", remote_path);
        if status.is_ok() {
            return Ok(());
        } else {
            return Err(Error::new(4, "Unable to upload"));

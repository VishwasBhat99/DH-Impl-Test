...
fn main() {
    ...
    let app_name = "cf-finnone-partial-loans";
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for Finnone Partial Loans.");
    info!(
        log,
        "Total duration for Finnone Partial Loans cashflow generation: `{:?}`.", total_duration
    );
    println!(
        "Total Duration for Finnone Partial Loans cashflow generation: {:2?}",
        total_duration
    );
}

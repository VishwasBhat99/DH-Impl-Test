...
pub struct OutputData {
    ...
    pub hft_rule_id: String,
    pub hft_charge_prcnt: String,
    pub yield_val: String,
    pub mduration_val: String,
    pub mar_val_weighted_mdur: String,
    pub price_sens_dur: String,
    pub spec_charge_amt_hcy: String,
    pub gen_market_risk_amt_hcy: String,
    pub tot_mr_amt_hcy: String,
    pub hft_amt_hcy: String,
...
impl Display for OutputData {
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
        write!(
            ...
            self.hft_rule_id,
            self.hft_charge_prcnt,
            self.yield_val,
            self.mduration_val,
            self.mar_val_weighted_mdur,
            self.price_sens_dur,
...
pub fn get_op_data(
    ...
    market_val_hcy: f64,
    isin: String,
    isin_map: &HashMap<String, ExtraData>,
    exchange_rate: f64,
    config_params: &ConfigurationParameters,
) -> OutputData {
    let spec_risk_cap_rule_id = spec_risk_prcnt[0..4].to_string();
    let spec_risk_cap_val = spec_risk_prcnt[4..8].to_string();
    ...
    let gen_mr_prcnt_val = gen_mr_prcnt[4..8].to_string();
    let gen_mr_prcnt = gen_mr_prcnt_val.parse::<f64>().unwrap_or(0.0) / 100.0;
    let hft_charge_prcnt_rule_id = hft_charge_prcnt[0..4].to_string();
    let hft_charge_prcnt_val = hft_charge_prcnt[4..8].to_string();
    let hft_charge_prcnt = hft_charge_prcnt_val.parse::<f64>().unwrap_or(0.0) / 100.0;
    let def_isin_data = ExtraData {
        yield_val: 0.0,
        mdur: 0.0,
    };
    let isin_data = isin_map.get(&isin).unwrap_or(&def_isin_data);
    let yield_val = isin_data.yield_val;
    let mduration_val = isin_data.mdur;
    let mar_val_weighted_mdur = market_val_hcy * mduration_val;
    let price_sens_dur = gen_mr_prcnt * mduration_val;
    let spec_charge_amt_hcy = (market_val_hcy * spec_risk_cap_prcnt) / 100.0;
    let gen_market_risk_amt_hcy = (price_sens_dur * market_val_hcy) / 100.0;
    let tot_mr_amt_hcy = spec_charge_amt_hcy + gen_market_risk_amt_hcy;
    let hft_amt_hcy = (hft_charge_prcnt * market_val_hcy) / 100.0;
    OutputData {
        ...
        hft_rule_id: hft_charge_prcnt_rule_id,
        hft_charge_prcnt: hft_charge_prcnt.to_string(),
        yield_val: yield_val.to_string(),
        mduration_val: mduration_val.to_string(),
        mar_val_weighted_mdur: mar_val_weighted_mdur.to_string(),
        price_sens_dur: price_sens_dur.to_string(),
        ...
        hft_amt_hcy: hft_amt_hcy.to_string(),
        add_prcnt_1: match account.get_string_for_key(&acc_keys.add_prcnt_1) {
            Ok(val) => val.to_string(),
            Err(_) => format!(""),
        },
        add_prcnt_2: match account.get_string_for_key(&acc_keys.add_prcnt_2) {
            Ok(val) => val.to_string(),
            Err(_) => format!(""),
        },
        add_prcnt_3: match account.get_string_for_key(&acc_keys.add_prcnt_3) {
            Ok(val) => val.to_string(),
            Err(_) => format!(""),
        },
        add_prcnt_4: match account.get_string_for_key(&acc_keys.add_prcnt_4) {
            Ok(val) => val.to_string(),
            Err(_) => format!(""),
        },
        add_prcnt_5: match account.get_string_for_key(&acc_keys.add_prcnt_5) {
            Ok(val) => val.to_string(),
            Err(_) => format!(""),
        },
        add_date_1: match account.get_string_for_key(&acc_keys.add_date_1) {
            Ok(val) => val.to_string(),
            Err(_) => format!(""),
        },
        add_date_2: match account.get_string_for_key(&acc_keys.add_date_2) {
            Ok(val) => val.to_string(),
            Err(_) => format!(""),
        },
        add_date_3: match account.get_string_for_key(&acc_keys.add_date_3) {
            Ok(val) => val.to_string(),
            Err(_) => format!(""),
        },
        add_date_4: match account.get_string_for_key(&acc_keys.add_date_4) {
            Ok(val) => val.to_string(),
            Err(_) => format!(""),
        },
        add_date_5: match account.get_string_for_key(&acc_keys.add_date_5) {
            Ok(val) => val.to_string(),
            Err(_) => format!(""),
        },
        add_str_1: match account.get_string_for_key(&acc_keys.add_str_1) {
            Ok(val) => val.to_string(),
            Err(_) => format!(""),
        },
        add_str_2: match account.get_string_for_key(&acc_keys.add_str_2) {
            Ok(val) => val.to_string(),
            Err(_) => format!(""),
        },
        add_str_3: match account.get_string_for_key(&acc_keys.add_str_3) {
            Ok(val) => val.to_string(),
            Err(_) => format!(""),
        },
        add_str_4: match account.get_string_for_key(&acc_keys.add_str_4) {
            Ok(val) => val.to_string(),
            Err(_) => format!(""),
        },
        add_str_5: match account.get_string_for_key(&acc_keys.add_str_5) {
            Ok(val) => val.to_string(),
            Err(_) => format!(""),
        },
    }

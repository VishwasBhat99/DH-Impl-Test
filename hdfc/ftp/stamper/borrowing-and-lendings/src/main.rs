...
fn main() {
    ...
    println!(
        "Total time to process borrowing and lending: {:?}",
        duration
    );
    let log_str = format!(
        "Total time to process borrowing and lending: {:?}",
...
fn initialize() -> FtpParameters {
    let app_name = "FTP-Stamper-Borr-Lend";
...
fn process_records(ftp_parameters: &mut FtpParameters) -> (String) {
    ...
    for mut account_input in ftp_parameters.input_data.iter() {
        ...
        //calculate FTP
        let (ftp_output, out_str, cf_out) = proc_ftp::calculate_ftp(
            &mut account_input,
            cf_input,
    ...
    match out_writer.write_all(output.as_bytes()) {
        Ok(_) => println!("Successfully processed all borrowing and lending accounts ."),
        Err(error) => panic!(

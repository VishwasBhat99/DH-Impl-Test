...
pub fn generate(config_params: &ConfigurationParameters, log: &Logger, diag_log: &Logger) {
    ...
    loop {
        ...
        if let Some(face_value) = input_account.face_value {
            tot_prin_in_in += face_value;
        }
        let cashflows_for_account_result = log_measurements!(
            diag_log,
            [format!(
                "Type: GenCFs, Identifier: `{}`",
                input_account.holding_deal_id
            )],
        ...
        tot_prin_in_op += account_with_cashflows.face_value;
        tot_int_in_op += account_with_cashflows.accrued_interest;
        log_measurements!(
            diag_log,
            [format!(
                "Type: WriteAccWithCFs, Identifier: {}",
                account_with_cashflows.holding_deal_id
            )],

...
fn main() {
    ...
    let app_name = "pp-pledged-depo-mabs";
    ...
    info!(
        log,
        "Total Duration taken by Overseas Pledged Deposits Pre-processor: {:?}", total_duration
    );
    println!(
        "Total Duration taken by Overseas Pledged Deposits Pre-processor: {:?}",
        total_duration
    );
}

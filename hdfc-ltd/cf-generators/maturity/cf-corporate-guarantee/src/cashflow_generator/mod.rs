...
pub fn generate(config_params: &ConfigurationParameters, log: &Logger, diag_log: &Logger) {
    ...
    loop {
        ...
        if let Some(currentoutstandingbal) = input_account.currentoutstandingbal {
            tot_prin_in_in += currentoutstandingbal;
        }
        ...
        tot_prin_in_op += account_with_cashflows.currentoutstandingbal;
        tot_int_in_op += 0.0;
        log_measurements!(
            diag_log,
            [format!(

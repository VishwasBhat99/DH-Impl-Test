...
pub fn process(config_params: ConfigurationParameters, log: &Logger, diag_log: &Logger) {
    ...
    let mut recon_writer = get_writer(config_params.recon_file_path());
    ...
    get_data(&dates, &config_params, &mut account_pool, log, &mut bal_org);
    ...
    get_op_line(
        ...
        &mut bal_org,
        &mut recon_writer,
        config_params.is_absolute(),
        config_params.source().to_string(),
    );

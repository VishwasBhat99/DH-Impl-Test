...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for OA OL.");
    info!(log, "Total Duration for OA OL: {:?}", total_duration);
    println!(
        "Total Duration for OA OL .cf file generator: {:2?}",
        total_duration
    );
}

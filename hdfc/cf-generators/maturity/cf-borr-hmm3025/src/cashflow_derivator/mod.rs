...
pub fn derive(input_file_path: &str, output_file_path: &str, as_on_dt: NaiveDate, log: &Logger) {
    ...
    loop {
        ...
        let oper_type_ip = &input_account.oper_type.to_uppercase();
        if oper_type_ip == "BORROWING" || oper_type_ip == "LENDING" {
            if oper_type_ip == "BORROWING" {
                tot_prin_in_ip += input_account.aip;
            } else if oper_type_ip == "LENDING" {
                tot_prin_in_ip += input_account.air;
            }
            ...
            let oper_type_op = a_w_cf.oper_type.to_uppercase();
            if oper_type_op == "BORROWING" {
                tot_prin_in_op += a_w_cf.aip;
            } else if oper_type_op == "LENDING" {
                tot_prin_in_op += a_w_cf.air;
            }
            ...
            log_error!(
                log,
                "`oper Type` is not well-formatted for account: `{}`. Hence ignoring the account.",

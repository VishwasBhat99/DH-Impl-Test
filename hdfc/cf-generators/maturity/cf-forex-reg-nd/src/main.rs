...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for Forex Register.");
    info!(
        log,
        "Total Duration for Forex Register: {:?}", total_duration
    );
    println!(
        "Total Duration for Forex Register cashflow file generator: {:2?}",
        total_duration
    );
}

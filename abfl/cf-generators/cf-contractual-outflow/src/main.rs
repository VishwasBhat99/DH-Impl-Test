...
fn main() {
    ...
    let app_name = "cf_contractual_outflow-1.0.2892";
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for Contractual Outflow.");
    info!(log, "Total Duration for Contractual Outflow: {:?}", total_duration);
    println!(
        "Total Duration for Contractual Outflow cashflow file generator: {:2?}",
        total_duration
    );
}

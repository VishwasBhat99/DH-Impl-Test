...
fn main() {
    ...
    let total_duration = end_generate_timer
        ...
        .expect("Could not calculate total duration for Encumbrance.");
    info!(
        log,
        "Total duration for Encumbrance cashflow generation: `{:?}`.", total_duration
    );
    println!(
        "Total Duration for Encumbrance cashflow generation: {:2?}",
        total_duration
    );
}

...
pub fn process(config_param: ConfigurationParameters, log: &Logger, diag_log: &Logger) {
    ...
    for (line_num, lines) in input_reader.deserialize().enumerate().skip(1) {
        ...
        tot_rec += 1;
        if input_account.cf_amt.parse::<f64>().is_err() {
            skp_rec += 1;
            continue;
        }
        let amt = input_account.cf_amt.parse().unwrap_or(DEFAULT_FLOAT);
        tot_amt += amt;
        ...
        let recon_key = ReconKey::new(
            input_account.cf_ccy,
            "fx_spot".to_string(),
            input_account.trsy_gl,
        );
        recon
            .store
            .entry(recon_key)
    ...
    output_writer(
        &mut recon_writer,
        recon.print(*config_param.as_on_date(), "FX Spot"),
    ...
    debug!(diag_log, "Writing FX Spot, Total Duration: {:?}.", duration);
}

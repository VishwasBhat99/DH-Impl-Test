...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for RD cashflow.");
    info!(
        log,
        "Total duration for RD cashflow generation: `{:?}`.", total_duration
    );
    println!(
        "Total Duration for RD cashflow generation: {:2?}",
        total_duration
    );
}

...
pub fn convert(
    ...
    if is_psl == true {
        let psl = input_account.psl.clone().parse::<i64>().unwrap_or(0) as usize;
        cfs_vec.sort_by(|a, b| b.date.cmp(&a.date));
        let op_cf_len = (cfs_vec.len() - psl) + 1;
        while cfs_vec.len() > op_cf_len {
            cfs_vec.remove(cfs_vec.len() - 1);
        }

        cfs_vec = cfs_vec.iter().rev().cloned().collect();
    }

...
pub fn get_op_line(
    ...
    let nfs = if out_acc.adim3_cmg.trim().to_uppercase() == "TD-NC-B".to_string() {
        "Y".to_string()
    } else if out_acc.adim3_cmg.trim().to_uppercase() == "TD-NC-R"
        && NaiveDate::parse_from_str(&int_rate_data.open_effective_date, "%d-%m-%Y")
            .unwrap_or(NaiveDate::from_ymd(1900, 1, 1))
            >= NaiveDate::from_ymd(2023, 12, 04)
    {
        "Y".to_string()
    } else {
        "N".to_string()

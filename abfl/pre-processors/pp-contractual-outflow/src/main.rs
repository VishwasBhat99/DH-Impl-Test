...
fn main() {
    ...
    let app_name = "pp_contractual_outflow";
    ...
    info!(
        log,
        "Total Duration for Contractual Outflow Pre-Processor: {:?}", total_duration
    );
    println!(
        "Total Duration for Contractual Outflow Pre-Processor: {:?}",
        total_duration
    );
}

...
impl ConfigurationParameters {
    pub fn log_parameters(&self, logger: &Logger) -> () {
        ...
        info!(logger, "is_contractual: {}", self.is_psl());
...
impl ConfigurationParameters {
    fn new_from_matches(matches: clap::ArgMatches) -> ConfigurationParameters {
        ...
        let is_psl = matches
            .value_of("is_psl")
            .expect("Error getting `is_psl` parameter value.")
            .parse::<bool>()
            .expect("Error parsing `is_psl` value as bool.");
...
fn get_eligible_arguments_for_app(app_name: &str) -> clap::ArgMatches {
    App::new(app_name)
        .about("This app generates Cashflows for Credit Card!")
        .version("1.0.3181")
        .author("saurabh.s <Saurabh Singh@surya-soft.com>")
        .arg(
        ...
        .arg(
            Arg::with_name("is_psl")
                .long("is-psl")
                .value_name("IS PSL")
                .possible_values(&["true", "false"])
                .help("This flag that decides whether to reduce the cashflows based on psl values or not.")
                .default_value("false")
                .required(false)
        )
        .arg(

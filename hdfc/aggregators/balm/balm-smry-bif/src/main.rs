...
fn main() {
    ...
    summary_gen::gen_summary(config_params, &log, &diagnostics_log);
    ...
    info!(
        log,
        "Total duration for Balm Summery generation: `{:?}`.", total_duration
    );
    println!(
        "Total duration for Balm Summery generation: {:2?}",
        total_duration
    );
}

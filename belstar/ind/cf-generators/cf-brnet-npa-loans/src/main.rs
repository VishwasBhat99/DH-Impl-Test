...
fn main() {
    ...
    let app_name = "npa-jayam-loans-cf";
    ...
    info!(
        log,
        "Total duration for NPA Jayam Loans cashflow generation: `{:?}`.", total_duration
    );
    println!(
        "Total Duration for NPA Jayam Loans cashflow generation: {:2?}",
        total_duration
    );
}

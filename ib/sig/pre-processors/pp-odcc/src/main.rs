...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for the ODCC Pre-Processor.");
    info!(
        log,
        "Total ODCC Pre-Processor Duration: {:?}", total_duration
    );
    println!("Total ODCC Pre-Processor Duration: {:?}", total_duration);
}

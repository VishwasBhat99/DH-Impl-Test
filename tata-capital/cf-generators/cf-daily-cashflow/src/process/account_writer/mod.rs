use macros;
use process::account_writer::account_indices::Index;
...
use super::maturity::AccountWithCashflows;
    ...
    pub fn write(&mut self, record: AccountWithCashflows) {
        ...
        if write_result.is_err() {
            panic!(
                ...
                record.acc_no,
                write_result.expect_err("Unexpected Error occured.")
            );
        }

        // Write the index for this account:
        self.account_index.set_key(record.acc_no);
...
fn flush_contents(mut writer: BufWriter<File>, writer_name: &str) {
    ...
    if flush_result.is_err() {
        panic!(
            ...
            writer_name,
            flush_result.expect_err("Unexpected error occured while flushing the buffer contents.")
        );
    }
}

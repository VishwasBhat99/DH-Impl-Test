...
pub fn process(config_param: ConfigurationParameters, log: &Logger, diag_log: &Logger) {
    ...
    if let Some(Ok(reader)) = input_file.worksheet_range(config_param.sheet_name()) {
        for mut row in reader.rows() {
            ...
            if skiprec == "DEALID"
                || skiprec == "TOTALOPERTYPE"
                || skiprec == "TOTALCURRENCY"
                || skiprec == "TOTALINSTRUMENT:"
                || skiprec == "GRAND_TOTAL:"
            {
                continue;
            }
            ...
            let oper_type_ip = fields[2].to_uppercase();
            let mut amt = 0.0;
            if oper_type_ip == "BORROWING" {
                amt = fields[18].parse::<f64>().unwrap_or(0.0);
            } else if oper_type_ip == "LENDING" {
                amt = fields[19].parse::<f64>().unwrap_or(0.0);
            }
            tot_amt += amt;
    ...
    let mut out_writer = match buf_file_wrtr(config_param.output_file_path(), None) {
        Ok(file) => file,
        Err(error) => panic!(
    ...
    debug!(
        diag_log,
        "Writing Etreasury borrowings HMM3025 records, Total Duration: {:?}.", duration
    );

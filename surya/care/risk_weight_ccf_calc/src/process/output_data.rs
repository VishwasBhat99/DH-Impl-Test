...
pub struct OutputData {
    ...
    pub ccf_rule_id: String,
    pub ccf_prcnt: f64,
    pub ccf_bal_lcy: String,
    pub ccf_bal_ccy: String,
    pub sys_rw: String,
...
pub fn get_op_data(
    ...
    let total_exposure = match account.get_f64_for_key(&acc_keys.total_exposure) {
        Ok(val) => val,
        Err(_) => 0.0,
    };
    let mut rw_out_bal_lcy = (ost_bal_lcy * ccf_prcnt) / 100.0;
    let rw_out_bal_ccy = if &acc_keys.total_exposure == "" {
        (ost_bal_ccy * ccf_prcnt) / 100.0
    } else {
        total_exposure
    };
    ...
    rw_out_bal_lcy = if prov_amt > rw_out_bal_lcy {
        0.0
    } else {
        rw_out_bal_lcy - prov_amt
    };
    if &acc_keys.total_exposure != "" {
        rw_out_bal_lcy = total_exposure;
    }
    ...
    OutputData {
        ...
        sys_sub_claim_rule_id: sub_id[4..8].to_string(),
        sys_classified_sub_claim_id: sub_id[4..8].to_string(),
        sys_rw_rule_id: rw_id[4..8].to_string(),
        ccf_rule_id: ccf_rule_id,
        ccf_prcnt: ccf_prcnt,
        ccf_bal_lcy: ((ost_bal_lcy * ccf_prcnt) / 100.0).to_string(),
        ccf_bal_ccy: ((ost_bal_ccy * ccf_prcnt) / 100.0).to_string(),

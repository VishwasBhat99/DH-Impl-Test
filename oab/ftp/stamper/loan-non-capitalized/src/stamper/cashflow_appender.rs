...
pub fn append_cashflow(
    ...
    cfoutput.field_type = match cfin.get_string_for_key(&input_field_names.field_type) {
        Ok(result) => result,
        Err(_e) => "",
    }
    .to_string();

    cfoutput.cf_principal_amount =
        match cfin.get_f64_for_key(&input_field_names.cf_principal_amount) {
            Ok(result) => result,
            Err(_e) => DEFAULT_FLOAT,
        };

    cfoutput.principal = match cfin.get_f64_for_key(&input_field_names.principal) {
        Ok(result) => result,
        Err(_e) => DEFAULT_FLOAT,
    };

    cfoutput.interest = match cfin.get_f64_for_key(&input_field_names.interest) {
        Ok(result) => result,
        Err(_e) => DEFAULT_FLOAT,
    };

    cfoutput.cf_date = match cfin.get_i64_for_key(&input_field_names.cf_date) {
        Ok(result) => result,
        Err(_e) => DEFAULT_INT,
    };
    ...
    cfoutput.int_compounding_frequency =
        match cfin.get_string_for_key(&input_field_names.int_compounding_frequency) {
            Ok(result) => result,
            Err(_e) => "",
        }
        .to_string();
    ...
    cfoutput.int_repayment_frequency =
        match cfin.get_string_for_key(&input_field_names.int_repayment_frequency) {
            Ok(result) => result,
            Err(_e) => "",
        }
        .to_string();
    ...
    cfoutput.pension_account_flag =
        match cfin.get_string_for_key(&input_field_names.pension_account_flag) {
            Ok(result) => result,
            Err(_e) => "",
        }
        .to_string();

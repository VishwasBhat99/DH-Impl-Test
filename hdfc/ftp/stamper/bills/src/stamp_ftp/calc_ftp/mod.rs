...
pub fn calc_ftp(
    ...
    match method {
        1001 => {
            ...
            let cpd_naive_date_time = NaiveDateTime::from_timestamp(cf_data_out.int_st_dt, 0);
            ...
            let (cf_data_out1, one_acc_op, cf_out) = ftp_calc_acc_level::calc_ftp(
                &mut acc_data_in,
                cf_data_out,
                inputfieldnames,
                basecurve,
            ...
            one_acc_op_new.bc_as_on_rule = cf_data_out.int_st_dt;
            one_acc_op_new.tenor_start_date_rule = cf_data_out.int_st_dt;
        ...
        1011 => {
            ...
            let (cf_data_out1, one_acc_op, cf_out) = ftp_calc_cflevel::calc_ftp(
                &mut acc_data_in,
                cf_data_out,
                inputfieldnames,
                basecurve,
        ...
        1031 => {
            ...
            //cpd: AsOn Date , TSD: AsOn Date, TED: Cashflow date
            if !(ftp_rates.is_empty()) && ftp_rates.contains_key(&cf_data_out.reference) {
                let rates = ftp_rates.get(&cf_data_out.reference).unwrap();
                let ted_naive_date_time = NaiveDateTime::from_timestamp(cf_data_out.mat_dt, 0);
                let ted = ted_naive_date_time.date();

                let (cf_data_out1, one_acc_op) = ftp_calc_lock::calc_ftp(
                    &mut acc_data_in,
                    cf_data_out,
                    inputfieldnames,
                    rates,
        ...
        1032 => {
            ...
            //CPD: AsOn Date , TSD: AsOn Date, TED: Maturity date
            if ftp_rates.contains_key(&cf_data_out.reference) {
                let rates = ftp_rates.get(&cf_data_out.reference).unwrap();
        ...
        1034 => {
            ...
            if !(ftp_rates.is_empty()) && ftp_rates.contains_key(&cf_data_out.reference) {
                ...
                if cpd <= NaiveDate::from_ymd(1970, 01, 01) {
                    let naive_date_time = NaiveDateTime::from_timestamp(cf_data_out.int_st_dt, 0);

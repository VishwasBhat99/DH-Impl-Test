...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for LCBG.");
    info!(log, "Total Duration for LCBG: {:?}", total_duration);
    println!(
        "Total Duration for LCBG .cf file generator: {:2?}",
        total_duration
    );
}

...
fn get_line(
    ...
    if val[5].to_string().parse::<f64>().is_err() {
        deal_dt = get_date(&val[5], "deal_date", &val[0], log)
    } else {
        deal_dt = datevalue_to_date(val[5].to_string());
    }

    let val_dt: String;
    if val[6].to_string().parse::<f64>().is_err() {
        val_dt = get_date(&val[6], "value_date", &val[0], log)
    } else {
        val_dt = datevalue_to_date(val[6].to_string());
    }

    let mat_dt: String;
    if val[14].to_string().parse::<f64>().is_err() {
        mat_dt = get_date(&val[14], "maturity_date", &val[0], log)
    } else {
        mat_dt = datevalue_to_date(val[14].to_string());
    }
    ...
    output_line.push('|');
    output_line.push_str(&remove_comma(val[11].to_string()).to_string());
    output_line.push('|');

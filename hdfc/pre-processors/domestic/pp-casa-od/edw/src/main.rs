...
fn main() {
    ...
    pre_processor::process(config_param, &diagnostics_log, &log);
    let end_main_timer = SystemTime::now();
    let total_duration = end_main_timer
        .duration_since(start_main_timer)
        .expect("Could not calculate UBS CASA OD total duration.");
    info!(log, "Total Duration for UBS CASA OD: {:?}", total_duration);
    println!("Total Duration for UBS CASA OD: {:2?}", total_duration);
}

...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for Securitisation Loans.");
    info!(
        log,
        "Total Duration for Securitisation Loans: {:?}", total_duration
    );
    println!(
        "Total Duration for Securitisation Loans: {:2?}",
        total_duration
    );
}

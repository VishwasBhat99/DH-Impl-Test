...
pub fn generate(config_params: &ConfigurationParameters, log: &Logger, diag_log: &Logger) {
    ...
    loop {
        ...
        if let Some(commitment_amt) = input_account.commitment_amt {
            tot_prin_in_in += commitment_amt;
        }
        ...
        tot_prin_in_op += account_with_cashflows.commitment_amt;
        tot_int_in_op += 0.0;
        log_measurements!(
            diag_log,
            [format!(
                "Type: WriteAccWithCFs, Identifier: {}",
                account_with_cashflows.br_no
            )],

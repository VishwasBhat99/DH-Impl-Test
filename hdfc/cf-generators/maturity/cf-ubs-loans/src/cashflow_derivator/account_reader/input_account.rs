...
pub struct InputAccount {
    ...
    pub call_option_date: Option<NaiveDate>,
    pub put_option_date: Option<NaiveDate>,
    pub is_acc_weaker: String,
    pub ews_weaker_value: String,
    pub frequency: String,
    pub gl_description: String,
    pub ratecode: String,
    pub ratespread: String,
    pub bdp_division: String,
    pub bdp_coa: String,
    pub retail: String,
    pub prod_desc: String,
    pub yldgrp_al: String,
    pub concat2_point: String,
    pub psl_category: String,
    pub lrd_udf: Option<NaiveDate>,
    pub nrd_udf: Option<NaiveDate>,
    pub sma_flag: String,
}
...
impl<'a> InputAccount {
    pub fn new_from_line(
        ...
        let input_account = InputAccount {
            ...
            spread_val: match value_iterator.next() {
                Some(val) => val.parse().unwrap_or(DEFAULT_FLOAT),

                None => {
                    return Err("Could not parse property `spread_val`.");
                }
            },
            ...
            call_option_date: match value_iterator.next() {
                Some(val) => dmy_date_parser.parse_opt(val),
                None => {
                    return Err("Could not parse property `call_option_date`.");
                }
            },
            put_option_date: match value_iterator.next() {
                Some(val) => dmy_date_parser.parse_opt(val),
                None => {
                    return Err("Could not parse property `put_option_date`.");
                }
            },
            ...
            gl_description: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `gl_description`.");
                }
            },
            ratecode: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `ratecode`.");
                }
            },
            ratespread: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `ratespread`.");
                }
            },
            ...
            bdp_coa: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `bdp_coa`.");
                }
            },
            retail: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `retail`.");
                }
            },
            ...
            yldgrp_al: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `yldgrp_al`.");
                }
            },
            concat2_point: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `concat2_point`.");
                }
            },
            psl_category: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `psl_category`.");
                }
            },
            lrd_udf: match value_iterator.next() {
                Some(val) => dmy_date_parser.parse_opt(val),
                None => {
                    return Err("Could not parse property `lrd_udf`.");
                }
            },
            nrd_udf: match value_iterator.next() {
                Some(val) => dmy_date_parser.parse_opt(val),
                None => {
                    return Err("Could not parse property `nrd_udf`.");
                }
            },

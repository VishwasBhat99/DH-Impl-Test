...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for Exchange Rate Pre-processor.");
    info!(
        log,
        "Total Duration for Exchange Rate Pre-processor: {:?}", total_duration
    );
    println!(
        "Total Duration for Exchange Rate Pre-processor: {:.2?}",
        total_duration
    );
}

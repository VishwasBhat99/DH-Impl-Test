...
pub fn process_name(
    ...
    let sign_factor = config_params.sign_factor();

    for (aggr_key, aggr_data) in acc_aggr.drain() {
        let mut total_bal_ccy = aggr_data.total_bal_ccy;
        let mut total_bal_lcy = aggr_data.total_bal_lcy;
        if *sign_factor == 1 {
            total_bal_ccy = total_bal_ccy * -1.0;
            total_bal_lcy = total_bal_lcy * -1.0;
        } else if *sign_factor == 2 {
            if total_bal_ccy < 0.0 {
                total_bal_ccy = total_bal_ccy * -1.0;
            }
            if total_bal_lcy < 0.0 {
                total_bal_lcy = total_bal_lcy * -1.0;
            }
        }
        let op = SummaryOutputData {
            as_on_date: config_params.as_on_date().format("%d-%m-%Y").to_string(),
            src_sys_code: config_params.src_sys_code().to_string(),
            ops_claim_id: aggr_key.llg_id[4..8].to_string(),
            out_bal_ccy: total_bal_ccy,
            out_bal_hcy: total_bal_lcy,
            currency: aggr_key.currency,
            op_rule_id: aggr_key.llg_id[0..4].to_string(),
        };

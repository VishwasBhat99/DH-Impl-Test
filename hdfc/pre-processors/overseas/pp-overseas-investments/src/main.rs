...
fn main() {
    ...
    info!(
        log,
        "Total Duration taken by Overseas Investments Pre-processor: {:?}", total_duration
    );
    println!(
        "Total Duration taken by Overseas Investments Pre-processor: {:?}",
        total_duration
    );
}

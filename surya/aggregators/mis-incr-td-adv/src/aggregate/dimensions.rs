...
pub fn get_dim(
    ...
    match dim_type {
        "NUMSLAB" => {
            ...
            let value = account
                .get_f64_for_key(key)
                .expect("Cannont read outstanding amount for an account.");
        ...
        "PRDSLAB" => {
            ...
            let st_dt = naivedate_from_timestamp(
                account
                    .get_i64_for_key(st_dt_key)
                    .expect("Cannot read start date for prd slab."),
            );
            let end_dt = naivedate_from_timestamp(
                account
                    .get_i64_for_key(end_dt_key)
                    .expect("Cannot read end date for prd slab."),
            );
        ...
        "SRCMAP" => {
            let key = &fields[0];
            let value = account
                .get_string_for_key(key)
                .expect("Cannont read src field for an account.");

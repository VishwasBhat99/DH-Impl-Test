...
fn main() {
    ...
    let app_name = "cf-non-ei-loans-ibk";
    ...
    let total_duration = end_generate_timer
        ...
        .expect("Could not calculate total duration for Non Ei Loans.");
    info!(
        log,
        "Total duration for Non Ei Loans cashflow generation: `{:?}`.", total_duration
    );
    println!(
        "Total Duration for Non Ei Loans cashflow generation: {:2?}",
        total_duration
    );
}

...
pub fn calc_yieldrate(
    ...
    let next_month_date = increment_date_by_months(date, 1);
    let monthly_days_diff = rbdate::num_days_start_to_end(date, next_month_date);
    ...
    for i in 0..lst_bm.len() {
        if !(i == lst_bm.len() - 1) {
            ...
            if days_diff >= lst_bm[i].days_diff && days_diff <= lst_bm[i + 1].days_diff {
                ...
                if days_diff <= monthly_days_diff {

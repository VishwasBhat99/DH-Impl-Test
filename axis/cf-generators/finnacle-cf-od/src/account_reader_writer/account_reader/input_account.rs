...
pub struct InputAccount {
    ...
    pub final_next_repricing_date: Option<NaiveDate>,
    pub npa_classification: String,
    pub cust_hlth_code: String,
    pub cust_npa_class: String,
    pub final_npa_class: String,
    pub final_foracid_suffix: String,
    pub final_foracid_prefix: String,
    pub npa_amount: f64,
}
...
impl<'a> InputAccount {
    pub fn new_from_line(
        ...
        let input_account = InputAccount {
            ...
            id_pref_pcnt: match value_iterator.next() {
                Some(val) => val.parse().unwrap_or(DEFAULT_FLOAT),
                None => {
                    return Err("Could not parse property `id_pref_pcnt`.");
                }
            },
            cust_pref_pcnt: match value_iterator.next() {
                Some(val) => val.parse().unwrap_or(DEFAULT_FLOAT),
                None => {
                    return Err("Could not parse property `cust_pref_pcnt`.");
                }
            },
            min_int_pcnt: match value_iterator.next() {
                Some(val) => val.parse().unwrap_or(DEFAULT_FLOAT),
                None => {
                    return Err("Could not parse property `min_int_pcnt`.");
                }
            },
            max_int_pcnt: match value_iterator.next() {
                Some(val) => val.parse().unwrap_or(DEFAULT_FLOAT),
                None => {
                    return Err("Could not parse property `max_int_pcnt`.");
                }
            },
            ...
            pegged_flg: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `pegged_flag`.");
                }
            },
            der_pegged_flg: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `der_pegged_flag`.");
                }
            },
            repricing_plan: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `repricing_plan`.");
                }
            },
            ...
            final_next_repricing_date: match value_iterator.next() {
                Some(val) => dmy_date_parser.parse_opt(val),
                None => {
                    return Err("Could not parse property `final_next_repricing_date`.");
                }
            },
            ...
            final_foracid_suffix: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `final_foracid_suffix`.");
                }
            },
            final_foracid_prefix: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `final_foracid_prefix`.");
                }
            },

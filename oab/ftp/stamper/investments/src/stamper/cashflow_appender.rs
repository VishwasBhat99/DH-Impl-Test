...
pub fn append_cashflow(
    ...
    cfoutput.prod_type = match cfin.get_string_for_key(&input_field_names.prod_type) {
        Ok(result) => result,
        Err(_e) => "",
    }
    .to_string();

    cfoutput.coup_rate = match cfin.get_f64_for_key(&input_field_names.coup_rate) {
        Ok(result) => result,
        Err(_e) => DEFAULT_FLOAT,
    };
    ...
    cfoutput.ctype = match cfin.get_string_for_key(&input_field_names.ctype) {
        Ok(result) => result,
        Err(_e) => "",
    }
    .to_string();

    cfoutput.acct_ng_type = match cfin.get_string_for_key(&input_field_names.acct_ng_type) {
        Ok(result) => result,
        Err(_e) => "",
    }
    .to_string();

    cfoutput.guarantor = match cfin.get_string_for_key(&input_field_names.guarantor) {
        Ok(result) => result,
        Err(_e) => "",
    }
    .to_string();

    cfoutput.ccode = match cfin.get_string_for_key(&input_field_names.ccode) {
        Ok(result) => result,
        Err(_e) => "",
    }
    .to_string();
    ...
    cfoutput.call_date = match cfin.get_i64_for_key(&input_field_names.call_date) {
        Ok(result) => result,
        Err(_e) => DEFAULT_INT,
    };

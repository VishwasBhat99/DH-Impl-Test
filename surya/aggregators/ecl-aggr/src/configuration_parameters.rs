...
impl ConfigurationParameters {
    fn new_from_matches(matches: clap::ArgMatches) -> ConfigurationParameters {
        ...
        let is_conversion_required = matches
            .value_of("is_conversion_required")
            .expect("Error while getting `is_conversion_required`.")
            .parse::<bool>()
            .expect("Error while parsing `is_conversion_required` as bool.");
        let exrt_file_path = matches
            .value_of("exrt_file_path")
            .expect("Error while getting `exrt_file_path`.")
            .to_string();
...
impl ConfigurationParameters {
    ...
    pub fn is_conversion_required(&self) -> bool {
        self.is_conversion_required
    }
...
fn get_eligible_arguments_for_app(app_name: &str) -> clap::ArgMatches {
    App::new(app_name)
        ...
        .arg(
            Arg::with_name("is_conversion_required")
                .long("is-conversion-required")
                .value_name("AMOUNT CONVERSION FLAG")
                .possible_values(&["true", "false"])
                .help("This flag that decides whether the amount will be converted to native amount .")
                .default_value("false")
                .required(false)
        )
        .arg(

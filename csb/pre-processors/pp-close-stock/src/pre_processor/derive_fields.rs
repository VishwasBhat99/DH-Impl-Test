...
pub fn get_op_line(
    ...
    if acc.cf_type.trim_matches('"').to_uppercase() == "PRINCIPAL" {
        prin_amt = &acc.cf_amt;
    } else if acc.cf_type.trim_matches('"').to_uppercase() == "INTEREST" {
        int_amt = &acc.cf_amt;
    }
    ...
    if alm_master.balm_llg == "NONE" {
        concats.push_str(&format!(
            "{}|{}|{}|{}\n",
            src_file_name, acc.instrument_id, acc.trsy_gl_cd, acc.book_val
        ));
    }
    op_line
}

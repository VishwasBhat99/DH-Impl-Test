...
fn main() {
    ...
    let app_name = "security_closing_stock";
    ...
    info!(
        log,
        "Total Duration taken by Security Closing Stock pre-processor: {:?}", total_duration
    );
    println!(
        "Total Duration taken by Security Closing Stock pre-processor: {:?}",
        total_duration
    );
}

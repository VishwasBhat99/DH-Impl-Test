...
impl<'a> InputAccount {
    pub fn new_from_line(
        ...
        let input_account = InputAccount {
            ...
            cod_acct_no: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `cod_acct_no`.");
                }
            },
            ...
            gl_account_principal: match value_iterator.next() {
                ...
                None => {
                    return Err("Could not parse property `gl_account_principal`.");
                }
            },
            gl_account_interest: match value_iterator.next() {
                Some(val) => val.parse().unwrap_or(DEFAULT_FLOAT),
                None => {
                    return Err("Could not parse property `gl_account_interest`.");
                }
            },
            gl_account_accrued: match value_iterator.next() {
                Some(val) => val.parse().unwrap_or(DEFAULT_FLOAT),
                None => {
                    return Err("Could not parse property `gl_account_accrued`.");
                }
            },
            ...
            eop_balance: match value_iterator.next() {
                Some(val) => val.parse().unwrap_or(DEFAULT_FLOAT),
                None => {
                    return Err("Could not parse property `eop_balance`.");
                }
            },
            index_rate: match value_iterator.next() {
                Some(val) => val.parse().unwrap_or(DEFAULT_FLOAT),
                None => {
                    return Err("Could not parse property `index_rate`.");
                }
            },
            net_rate: match value_iterator.next() {
                Some(val) => val.parse().unwrap_or(DEFAULT_FLOAT),
                None => {
                    return Err("Could not parse property `net_rate`.");
                }
            },
            benchmark_name: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `benchmark_name`.");
                }
            },
            ...
            npa_status: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `npa_status`.");
                }
            },
            npa_status_final: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `npa_status_final`.");
                }
            },
            ptc_flag: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property ptc_flag.");
                }
            },
            foreclosure: match value_iterator.next() {
                Some(val) => val.parse().unwrap_or(DEFAULT_FLOAT),
                None => {
                    return Err("Could not parse property foreclosure.");
                }
            },
            foreclosure_rate_1: match value_iterator.next() {
                Some(val) => val.parse().unwrap_or(DEFAULT_FLOAT),
                None => {
                    return Err("Could not parse property foreclosure_rate_1.");
                }
            },
            foreclosure_rate_2: match value_iterator.next() {
                Some(val) => val.parse().unwrap_or(DEFAULT_FLOAT),
                None => {
                    return Err("Could not parse property foreclosure_rate_2.");
                }
            },
            index_code: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property index_code.");
                }
            },
            derived_rt_type: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property derived_rt_type.");
                }
            },
            ...
            arrear_prin: match value_iterator.next() {
                Some(val) => val.parse().unwrap_or(DEFAULT_FLOAT),
                None => {
                    return Err("Could not parse property arrear_prin.");
                }
            },
            add_string_1: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property add_string_1.");
                }
            },
            add_string_2: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property add_string_2.");
                }
            },
            add_int_1: match value_iterator.next() {
                Some(val) => val.parse().unwrap_or(DEFAULT_INT),
                None => {
                    return Err("Could not parse property add_int_1.");
                }
            },
            add_int_2: match value_iterator.next() {
                Some(val) => val.parse().unwrap_or(DEFAULT_INT),
                None => {
                    return Err("Could not parse property add_int_2.");
                }
            },
            add_float_1: match value_iterator.next() {
                Some(val) => val.parse().unwrap_or(DEFAULT_FLOAT),
                None => {
                    return Err("Could not parse property add_float_1.");
                }
            },
            add_float_2: match value_iterator.next() {
                Some(val) => val.parse().unwrap_or(DEFAULT_FLOAT),
                None => {
                    return Err("Could not parse property add_float_2.");
                }
            },
    ...
    pub fn new_from_acc(inp_acc: InputAccount) -> InputAccount {
        inp_acc
    }
}

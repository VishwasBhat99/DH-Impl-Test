...
pub struct InputAccount {
    ...
    pub inr_amt: f64,
    pub inr_rt: f64,
    pub cont_usage: String,
    pub non_idx_fwcurcy: String,
    pub non_idx_fwamt: f64,
    pub indxn_dt: Option<NaiveDate>,
}
...
impl<'a> InputAccount {
    pub fn new_from_line(
        ...
        let input_account = InputAccount {
            ...
            cont_usage: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `cont_usage`.");
                }
            },
            non_idx_fwcurcy: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `non_idx_fwcurcy`.");
                }
            },
            non_idx_fwamt: match value_iterator.next() {
                Some(val) => val.parse().unwrap_or(DEFAULT_FLOAT),
                None => {
                    return Err("Could not parse property `non_idx_fwamt`.");
                }
            },
            indxn_dt: match value_iterator.next() {
                Some(val) => dmy_date_parser.parse_opt(val),
                None => {
                    return Err("Could not parse property `indx_dt`.");
                }
            },
        };
        Ok(input_account)

...
fn main() {
    ...
    let app_name = "finnone-loan-pp-1.0.1";
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate Total Finnone Loans Pre-Processor duration.");
    info!(
        log,
        "Total Time Taken By Finnone Loans Pre-Processor: {:?}", total_duration
    );
    println!(
        "Total Time Taken By Finnone Loans Pre-Processor: {:#?}",
        total_duration
    );
}

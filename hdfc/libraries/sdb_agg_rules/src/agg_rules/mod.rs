...
impl AggRulesBuilder {
    fn new_from_path(path: &str, reader: &Reader) -> AggRules {
        ...
        for line in rdr.lines() {
            rules.build_with_intermediate_rule_step(
                IntermediateRuleStep::new_from_line(line.unwrap(), reader)
            );
        }
    ...
    fn build(self) -> AggRules {
        ...
        ord_rules.sort_by(|curr, next| curr.sequence.partial_cmp(&next.sequence).expect("Couldn't compare?"));

...
impl AddAssign for PrincipalInterestAmountReport {
    fn add_assign(
        &mut self,
        other: PrincipalInterestAmountReport
    ) {
...
impl LLGsReport {
    ...
    pub fn add_account_totals_for_llg(
        &mut self,
        llg: &LLGKey,
        totals: InputReport
    ) {
        if self.report.contains_key(llg) {
            self.report.get_mut(llg).unwrap()
                .add_account_total(totals)
        } else {
    ...
    pub fn add_aggregate_writer_report(
        ...
        let mut output_records_written_report = [0; 6];
        ...
        OutputRecordsWrittenReport::new(
            output_records_written_report
        )
    }
}

pub type GroupAmountsReport = [PrincipalInterestAmountReport; 6];
...
impl LLGReport {
    fn new() -> LLGReport {
        LLGReport {
            ...
            six_file_amounts: [
                PrincipalInterestAmountReport::new(); 6
            ]
        }
    }

    fn add_account_total(
        &mut self,
        account_total: InputReport
    ) {
...
impl Serialize for LLGReport {
            ...
            S: Serializer,
    {
        ...
        s.serialize_field("cashflowsCount", &self.accounts_total_report.cashflows_count)?;
        s.serialize_field("totalPrincipalAmount", &self.accounts_total_report.total_principal_amount)?;
        s.serialize_field("totalInterestAmount", &self.accounts_total_report.total_interest_amount)?;;

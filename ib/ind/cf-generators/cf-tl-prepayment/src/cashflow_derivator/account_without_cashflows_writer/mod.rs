...
use cashflow_derivator::account_without_cashflows_writer::account_indices::Index;
use macros;
...
impl AccountWithoutCashflowsWriter {
    pub fn write(&mut self, record: AccountWithoutCashflows) {
        // Write the account
        ...
        self.account_index.set_key(record.customer_no);
...
pub fn flush_contents(mut writer: BufWriter<File>, writer_name: &str) {
    ...
    if flush_result.is_err() {
        panic!(
            ...
            writer_name,
            flush_result.expect_err("Unexpected error occured while flushing buffer!")
        );
    }
}

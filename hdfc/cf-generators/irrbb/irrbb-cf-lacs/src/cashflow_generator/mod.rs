...
pub fn generate(config_params: &ConfigurationParameters, log: &Logger, _diag_log: &Logger) {
    ...
    for mut account_with_cfs in account_reader.iter() {
        ...
        out_acc.emi_overdue_gl_cd = data_vec[63].as_integer().unwrap_or(0);
        out_acc.pre_emi_overdue_gl_cd = data_vec[64].as_integer().unwrap_or(0);
        out_acc.excess_emi_gl_cd = data_vec[65].as_integer().unwrap_or(0);
        out_acc.excess_pre_emi_gl_cd = data_vec[66].as_integer().unwrap_or(0);
        out_acc.tot_prin_amt = data_vec[67].as_float().unwrap_or(0.0);
        out_acc.tot_int_amt = data_vec[68].as_float().unwrap_or(0.0);
        out_acc.asondate = data_vec[69].as_integer().unwrap_or(0);
        ...
        let next_rep_date = out_acc.next_rep_date;
        ...
        if highest_date == timestamp(date_parser.parse("01-01-1099")) {
            let last_cf_date = out_acc.emi_last_paid_date_crnt;
        ...
        final_cf.date = out_acc.next_rep_date;
...
fn create_io_workers(output_path: &str, log: &Logger) -> AccountWithCashflowsWriter {
    let output_path_str = format!("{}.cf", output_path);

    ...
    pub fn does_acc_abide(&self, account: &AccountWithCFs) -> bool {
        match &self.comparator {
            ...
            Comparator::BTWN(expected_vals) => {
                ...
                match expected_type {
                    Type::I32 => match (&expected_vals.0, &expected_vals.1) {
                        ...
                        _ => panic!("Comparison attempted between values of different dataype ! !"),
                    },
            ...
            Comparator::IN(expected_val) => match expected_val {
                Value::String(e_v) => {
                    let acc_val = match account.get_string_for_key(&self.field_name) {
                        Ok(val) => val.to_string(),
                        Err(_) => match account.get_i64_for_key(&self.field_name) {
                            Ok(val) => val.to_string(),
                            Err(_) => match account.get_i32_for_key(&self.field_name) {
                                Ok(val) => val.to_string(),
                                Err(_) => match account.get_f64_for_key(&self.field_name) {
                                    Ok(val) => val.to_string(),
                                    Err(_) => match account.get_f32_for_key(&self.field_name) {
                                        Ok(val) => val.to_string(),
                                        // Using NA as default value for empty value
                                        Err(_) => "NA".to_string(),
                                    },
                                },
                            },

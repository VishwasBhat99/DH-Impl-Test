...
impl InputAccountReader {
    pub fn new(src_file: &str, log: &Logger) -> InputAccountReader {
        let dmy_dt_parser = DateParser::new("%d-%m-%Y".to_string(), false);
        ...
        while !correct_record_found {
            ...
            if let Some(line) = line_opt {
                ...
                if let Ok(underlying_string) = line {
                    ...
                    let parse_result =
                        InputAccount::new_from_line(underlying_string, &self.dmy_dt_parser);
                    ...
                    if parse_result.is_err() {
                        ...
                        log_error!(
                            ...
                            parse_result
                                .err()
                                .expect("Unexpected error occured while reading account.")
                        );
                        continue;
                    }

                    next_account = Some(parse_result.expect("Error while getting next record."));

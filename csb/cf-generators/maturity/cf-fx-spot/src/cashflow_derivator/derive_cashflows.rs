...
pub fn derive_cashflows(
    ...
    if amt < 0.0 {
        ...
        let negative_o_a_cf = new_cashflow(account.int_amt, amt, cf_dt);
    ...
    // Case: cf_dt < as_on_dt
    if cf_dt < as_on {
        log_info!(log, "`overdue` for account: `{}`.", account.deal_ref,);
    }

    let int_amt = account.int_amt;

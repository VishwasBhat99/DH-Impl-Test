...
fn main() {
    ...
    println!("Total time to process bullion Loans: {:?}", duration);
    let log_str = format!("Total time to process bullion Loans: {:?}", duration);
...
fn initialize() -> FtpParameters {
    let app_name = "FTP-Stamper-Bullion-Loans";
...
fn process_records(ftp_parameters: &mut FtpParameters) -> (String) {
    ...
    for mut account_input in ftp_parameters.input_data.iter() {
        ...
        total_bal += cf_input.tot_prin_amt;
    ...
    match out_writer.write_all(output.as_bytes()) {
        Ok(_) => println!("Successfully processed all bullion loans accounts ."),
        Err(error) => panic!(
    ...
    match out_writer.write_all(cashflow_out.as_bytes()) {
        Ok(_) => println!("Successfully cashflow output for all bullion loans accounts ."),
        Err(error) => panic!(
            "Unable to write processed lines on file `{}`.{}",
            out_path, error
        ),

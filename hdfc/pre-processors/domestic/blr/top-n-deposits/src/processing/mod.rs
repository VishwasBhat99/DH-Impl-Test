...
pub fn process(config_params: &ConfigurationParameters, log: &Logger, diag_log: &Logger) {
    ...
    let top_n_dep_data = read_file_and_create_op_data(
        &config_params,
        &mut ttl_accnts,
        &mut ttl_suc_accnts,
        &mut ttl_bal_ip,
    ...
    info!(
        diag_log,
        "Reading deposits file & creating top n deposits output data, Total Duration: {:?}.",
        duration
    );

    start_timer = SystemTime::now();
    // Writing output data into file.
    write_output(&config_params, top_n_dep_data, &diag_log);
    ...
    info!(
        diag_log,
        "Writing top n deposits data into File, Total Duration: {:?}.", duration
    );

    ...
    pub fn new_from_line(
        ...
        let input_account = InputAccount {
            ...
            cntrct_usage: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `contract usage`.");
                }
            },
            non_idx_flw_curr: match value_iterator.next() {
                Some(val) => val.to_string(),
                None => {
                    return Err("Could not parse property `non index flow currency`.");
                }
            },
            non_idx_cf_amt: match value_iterator.next() {
                Some(val) => val.parse().unwrap_or(DEFAULT_FLOAT),
                None => {
                    return Err("Could not parse property `non index cf amount`.");
                }
            },
            indexation_dt: match value_iterator.next() {
                Some(val) => dmy_date_parser.parse_opt(val),
                None => {
                    return Err("Could not parse property `indexation date`.");
                }
            },
        };
        Ok(input_account)

...
fn main() {
    ...
    let app_name = "cf-forward-derivatives-1.0.0";
    ...
    let total_duration = end_generate_timer
        ...
        .expect("Could not calculate total duration for Forward Derivative.");
    info!(
        log,
        "Total duration for Forward Derivative cashflow generation: `{:?}`.", total_duration
    );
    println!(
        "Total Duration for Forward Derivative cashflow generation: {:2?}",
        total_duration
    );
}

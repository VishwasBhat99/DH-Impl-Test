...
pub fn create_account_with_cashflows(
    ...
    op_acc.trade_date = if let Some(dt) = account.trade_date {
        timestamp(dt)
    } else {
        DEFAULT_INT
    };
    op_acc.ndf_settlementcurrency = account.ndf_settlementcurrency;
    op_acc.mat_date_of_trade = if let Some(dt) = account.mat_date_of_trade {
        timestamp(dt)
    } else {
        DEFAULT_INT
    };

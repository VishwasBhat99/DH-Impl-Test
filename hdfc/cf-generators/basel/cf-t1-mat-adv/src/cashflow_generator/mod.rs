...
pub fn generate(config_params: &ConfigurationParameters, log: &Logger, diag_log: &Logger) {
    ...
    for (line_num, lines) in cust_master_file.lines().enumerate() {
        ...
        let fields: Vec<&str> = line.split(config_params.cust_master_sep()).collect();
        if !cust_details.contains_key(&fields[0].trim().to_string()) {
            continue;
        }
    ...
    for (line_num, lines) in cust_file.lines().enumerate().skip(1) {
        ...
        let fields: Vec<&str> = line.split(config_params.cust_sep()).collect();
        if line_num == 1 {
            info!(
                log,
                "{:?} columns present in Cust File, Atleast 2 columns expected",
                fields.len()
            );
        }
    ...
    for (line_num, lines) in biu_file.lines().enumerate() {
        ...
        if line_num == 0 {
            info!(
                log,
                "{:?} columns present in BIU File, Atleast 6 columns expected",
                fields.len()
            );
        }
        if let Some(mut cust_fields) = cust_details.remove(fields[0]) {
    ...
    for (line_num, lines) in cust_id_file.lines().enumerate().skip(1) {
        ...
        let fields: Vec<&str> = line.split(config_params.cust_id_sep()).collect();
        if line_num == 1 {
            info!(
                log,
                "{:?} columns present in Cust-ID File, Atleast 4 columns expected",
                fields.len()
            );
        }
        if let Some(mut cust_fields) = cust_details.remove(fields[0]) {
    ...
    for (line_num, lines) in class_file.lines().enumerate() {
        ...
        if line_num == 0 {
            info!(
                log,
                "{:?} columns present in Class File, Atleast 45 columns expected",
                fields.len()
            );
        }

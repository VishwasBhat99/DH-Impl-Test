...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for Option Register.");
    info!(
        log,
        "Total Duration for Option Register: {:?}", total_duration
    );
    println!(
        "Total Duration for Option Register cashflow file generator: {:2?}",
        total_duration
    );
}

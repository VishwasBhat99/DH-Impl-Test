...
fn main() {
    ...
    let app_name = "cf-loans-merger-irrbb";
    ...
    info!(
        log,
        "Total duration for Loans Merger cashflow generation: `{:?}`.", total_duration
    );
    println!(
        "Total Duration for Loans Merger cashflow generation: {:2?}",
        total_duration
    );
}

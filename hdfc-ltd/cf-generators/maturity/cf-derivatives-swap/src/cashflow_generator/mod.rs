...
pub fn generate(config_params: &ConfigurationParameters, log: &Logger, diag_log: &Logger) {
    ...
    loop {
        ...
        if let Some(outstanding_notional_rec_leg_lcy) =
            input_account.outstanding_notional_rec_leg_lcy
        {
            tot_prin_in_in += outstanding_notional_rec_leg_lcy;
        }
        ...
        tot_prin_in_op += account_with_cashflows.outstanding_notional_rec_leg_lcy;
        tot_int_in_op += account_with_cashflows.outstanding_notional_pay_leg_lcy;
        log_measurements!(
            diag_log,
            [format!(
                "Type: WriteAccWithCFs, Identifier: {}",
                account_with_cashflows.deal_id
            )],

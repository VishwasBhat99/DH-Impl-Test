...
impl CashflowAggregatedOnDateBuilder {
    ...
    pub fn add_irs_cf(&mut self, cf: &Cashflow, interest_rate: f64, day_num: i64) {
        self.add_irs(cf.get_principal_amount(), interest_rate, day_num as f64);
    }
    ...
    pub fn to_cf_aggregated(self) -> LLGAggregateOnDay {
        ...
        let slr = {
            if self.total_principal_amount == 0.0 {
                CashflowAggregated {
                    amount: 0.0,
                    rate: 0.0,
                    date: 0.0,
...
impl CashflowAggregated {
    ...
    pub fn to_string(&self) -> String {
        format!("{:.2}|{:.2}|{:.2}|", self.amount, self.rate, self.date)
    }

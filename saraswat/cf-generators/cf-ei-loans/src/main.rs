...
fn main() {
    ...
    let total_duration = end_generate_timer
        ...
        .expect("Could not calculate total duration for Ei Loans.");
    info!(
        log,
        "Total duration for Ei Loans cashflow generation: `{:?}`.", total_duration
    );
    println!(
        "Total Duration for Ei Loans cashflow generation: {:2?}",
        total_duration
    );
}

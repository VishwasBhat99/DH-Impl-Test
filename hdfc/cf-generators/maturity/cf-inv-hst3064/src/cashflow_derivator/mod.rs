...
pub fn derive(
    ...
    loop {
        ...
        tot_prin_in_ip += input_account.amt;

        let cashflows = log_measurements!(
            diag_log,
            [format!(
                "Type: DeriveCFs, Identifier: {}",
                input_account.security_desc
            )],
            derive_cashflows(&input_account, as_on_dt, log)
        );

        total_accounts_with_cashflows += 1;
        total_cfs += cashflows.len();

        let a_w_cf = create_account_with_cashflows(input_account, as_on_dt, cashflows);
        tot_prin_in_op += a_w_cf.amt;

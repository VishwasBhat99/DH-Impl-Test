...
pub fn calc_ftp_cflevel(
    ...
    if Path::new(&full_file_path).exists() {
            ...
            .iter_mut()
        {
            ...
            let base_rate_prod = yield_rate * residual_days as f64 * cf_obj.principal_amount;
            let end_rate_prod =
                (yield_rate + adj_rate) * residual_days as f64 * cf_obj.principal_amount;
            let org_bal_tenor = residual_days as f64 * cf_obj.principal_amount;
    ...
    cf_data_out.total_balance = half_away_from_zero(cf_data_out.total_balance, bal_precision);
    cf_data_out.total_interest_ftp =
        half_away_from_zero(cf_data_out.total_interest_ftp, bal_precision);
    total_org_tenor_bal = half_away_from_zero(total_org_tenor_bal, bal_precision);
    total_end_rate_prod = half_away_from_zero(total_end_rate_prod, bal_precision);
    total_base_rate_prod = half_away_from_zero(total_base_rate_prod, bal_precision);
    ...
    for i in 0..fix_lst_adjustments.len() {
        ...
        fix_lst_adjustments[i], st_dt, cf_data_out.acc_no
    );
                    DEFAULT_FLOAT
                }
            },
    ...
    match lock_type {
        ...
        "TPR" => {
            spread = cf_data_out.int_rt - cf_data_out.ftp_rate;
        }
        _ => {
    ...
    spread = half_away_from_zero(spread, rate_precision);
    if is_lock {
        let ftp_rates_out = format!(
            "{}|{}|{}|{}|{}{}|{}|{}\n",
            cf_data_out.ubs_acct_num,
            cf_data_out.ftp_rate,
            basecurve,
            base_rate,
            adj_lock_str,
        ...
        out_str = format!(
            ...
            spread,
            variable_adj_rate,
            half_away_from_zero((spread + variable_adj_rate), rate_precision)
        );
    } else {
        out_str = format!(
            "{}{}|{}|{}|",
    ...
    (cf_data_out, out_str, cf_str, one_acc_op)
}

...
impl ConfigurationParameters {
    fn new_from_matches(matches: clap::ArgMatches) -> ConfigurationParameters {
        ...
        let rules_file_path = matches
            ...
            .to_string();
        let threshold_file_path = matches
            .value_of("threshold_file_path")
            .expect("Error getting `threshold_file_path`.")
            .to_string();
...
impl ConfigurationParameters {
    ...
    pub fn threshold_file_path(&self) -> &str {
        &self.threshold_file_path
    }
    ...
    App::new(app_name)
        ...
        .arg(
            Arg::with_name("is_consolidated")
                ...
                .possible_values(&["true", "false"])
                .help("This flag that decides whether amount in input is in native currency or not.")
                .default_value("false")
                .required(false)
        )
        .arg(
        ...
        .arg(
            Arg::with_name("threshold_file_path")
                .long("threshold-file-path")
                .value_name("THRESHOLD-FILE-PATH")
                .help("The path to the file that contains the threshold for each year in input data.")
                .required(true)
        )
        .arg(
        ...
        .arg(
            Arg::with_name("is_amt_abs")
                .long("is-amt-abs")
                .value_name("IS AMOUNT ABSOLUTE")
                .possible_values(&["true", "false"])
                .help("This flag decides whether principal amount in to be taken as absolute or not")
                .default_value("true")
                .required(false)
        )
        .get_matches()

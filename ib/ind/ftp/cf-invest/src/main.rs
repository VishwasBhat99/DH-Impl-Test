...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for CC Emi.");
    info!(log, "Total Duration for CC EMI cf: {:?}", total_duration);
    println!(
        "Total Duration for Investment cashflow file generator: {:2?}",
        total_duration
    );
}

                ...
                match Account::new_from_line(each_line.to_owned()) {
                    ...
                    Ok(account) => {
                        ...
                        let base_curr_amt = grp_amt_map
                            .get(&(grp_id.to_string() + &"|".to_string() + &"RUP".to_string()))
                            .expect("Cannot find grp currency combination in map");

                        let perc_limit = (base_curr_amt * config_params.calc_percentage()) / 100.0;
                        if config_params.calc_type() == "ABSOLUTE" {
                            limit = (llg_amt / base_curr_amt) * limit;
                        }
                        if config_params.calc_type() == "PERCENTAGE" {
                            limit = (llg_amt / base_curr_amt) * perc_limit;
                        }
                        if config_params.calc_type() == "ABSOLUTE-PERCENTAGE" {
                            if limit > perc_limit {
                                limit = (llg_amt / base_curr_amt) * limit;
                            } else {
                                limit = (llg_amt / base_curr_amt) * perc_limit;

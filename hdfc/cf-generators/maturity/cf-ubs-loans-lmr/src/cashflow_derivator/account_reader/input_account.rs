...
pub struct InputAccount {
    ...
    pub npa_typ: String,
    pub bmid: String,
    pub lcy_out_amt_usd: f64,
    pub lcy_int_amt_usd: f64,
    pub penalty_amt_usd: f64,
    pub int_rate: f64,
    pub int_payout_dt: Option<NaiveDate>,
}
...
impl<'a> InputAccount {
    pub fn new_from_line(
        ...
        let input_account = InputAccount {
            ...
            lcy_out_amt_usd: match value_iterator.next() {
                Some(val) => val.parse().unwrap_or(DEFAULT_FLOAT),

                None => {
                    return Err("Could not parse property `lcy_out_amt_usd`.");
                }
            },
            lcy_int_amt_usd: match value_iterator.next() {
                Some(val) => val.parse().unwrap_or(DEFAULT_FLOAT),

                None => {
                    return Err("Could not parse property `lcy_int_amt_usd`.");
                }
            },
            penalty_amt_usd: match value_iterator.next() {
                Some(val) => val.parse().unwrap_or(DEFAULT_FLOAT),

                None => {
                    return Err("Could not parse property `penalty_amt_usd`.");
                }
            },
            ...
            int_payout_dt: match value_iterator.next() {
                Some(val) => dmy_date_parser.parse_opt(val),
                None => {
                    return Err("Could not parse property `last repricing date`.");
                }
